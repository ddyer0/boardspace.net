...
{ my ($dbh,$uid,$fromname,$passwd,$historic,$tid,$admin) = @_;
  ...
  my $q = "SELECT status,uid,matchtype,description,count(pid),sum(if(pid=$qu,1,0)),start,longdescription from tournament left join participant "
...
#message print "tournament $q<p>";
  ...
  if($n>0)
    { print "<table><tr><td>";
	  ...
	  . "</b></td></tr>\n";
    while($n-->0)
      {my ($thisstat,$thistid,$matchtype,$messid,$nplayers,$myplayers,$start,$longform) = &nextArrayRow($sth);
       print "<tr>";
       $mat = $matchtype;
	 ...
	 {	# show details for a particular tournament
		 &show_current_matches($dbh,$tid,$mat,$admin);
	 }
   ...
   { if($tournamentid>0)
     ...
     if($subscribe)
       ...
       {my $q = "delete from participant where tid=$qtid and pid=$quid";
         &commandQuery($dbh,$q);  
		my $q2 = "delete from matchrecord where player1=$quid and tournament=$qtid and matchstatus='notready'";
		&commandQuery($dbh,$q2);
...
{	my ($row,$tid,$matchtype,$group,@ids) = @_;
	print "<tr>";
	print "<td>";
	...
	print "</td>";
	if($matchtype eq 'matchparticipant')
	{
	#up to 6 players
	print "<td>";
	&print_player_selector("player3_$row",0,"",@ids);
	print "</td><td>";
	&print_player_selector("player4_$row",0,"",@ids);
	print "</td>";
	print "<td>";
	&print_player_selector("player5_$row",0,"",@ids);
	print "</td><td>";
	&print_player_selector("player6_$row",0,"",@ids);
	print "</td>";
...
sub show_filled_match_row()
{	my ($admin,$closed,$row,$matchid,$pl1,$name1,$outcome1,$player1_points,$played,$comment,@ids) = @_;
	...
	if($admin)
	{
	print "<td>del <input type=checkbox name=delete_$row>\n";
	print "<br>email <input type=checkbox name=email_$row>\n";
	print "</td>";
	print "<td><a href=$ENV{'SCRIPT_NAME'}?operation=editmatch&fromname=$plname1&t2admin=$admin&matchid=$matchid>edit</a>";	
	print "</td>";
	...
	my $p1msg = ($closed || ($pl1 == -1)) ? "<td>$name1</td>"
		:"<td><a href=$ENV{'SCRIPT_NAME'}?operation=editmatch&fromname=$plname1&matchid=$matchid>$name1</td>";
	print $p1msg;
	}

	{
	print "<td>\n";
	my $outcome = &resultString($outcome1,$name1);
	print "$outcome";
	print "</td>";
	print "<td>$played";
	print "</td>";
	...
	print "<td>";
	if($comment) { print "<b>$name1</b>: $comment<br>"; };			
	print "<input type=hidden name=match_$row value=$matchid>";
	print "</td>";
	print "</tr>\n"						
}
sub show_filled_participant_match_form()
{	my ($admin,$closed,$row,$matchid,$pl1,$name1,
		$outcome1,$points,$played,$comment,@ids) = @_;
	my $en1 = &uri_escape($name1);
	my $plname1 = $names{$pl1};
	print "<tr><td>\n";
	if($admin)
	...
	{
	print "<td>\n";
	my $outcome1r = &resultString($outcome1,$name1);
	print "$outcome1r";
	print "</td><td>";
	print "$played";
	}
...
sub show_matches_in_participant_group()
{	my ($dbh,$tournamentid,$matchtype,$group,$type,$status,$mcomment,$sortkey,$admin) = @_;
	...
	{
    &print_form_header();
	my $createop = ($matchtype eq 'matchparticipant') ? 'createparticipantmatch' : 'creatematch';
	print "<table>\n";
	...
	{
	my $q = "select player,outcome,points,played,comment,matchid from matchparticipant"
		. " where (matchstatus!='notready') and tournament=$qturn and tournament_group=$qgroup order by $or";
	my $sth = &query($dbh,$q);
	my $nr = &numRows($sth);
	#
	...
	while($nr-- > 0)
	{
	my ($pl1,$outcome1,$points,$outcome,$played,$comment,$matchid) = &nextArrayRow($sth);
	my $name1 = $names{$pl1};
	$hasmatch{$pl1} = 1;
	if($admin)
	{
	...
	$found{$pl1}=$pl1;
	&show_filled_participant_match_form($admin,$closed,$nrows++,$matchid,$pl1,$name1,$outcome1,
							$points,$played,$comment,@ids);
	}
	...
	#
	&show_blank_match_form($nrows++,$tournamentid,$matchtype,$group,@ids);
	&show_blank_match_form($nrows++,$tournamentid,$matchtype,$group,@ids);
	&show_blank_match_form($nrows++,$tournamentid,$matchtype,$group,@ids);
	&show_blank_match_form($nrows++,$tournamentid,$matchtype,$group,@ids);
	print "</table>";
...
{	my ($dbh,$tournamentid,$matchtype,$group,$type,$status,$mcomment,$sortkey,$admin) = @_;
	...
	my $q = "select player,comment,matchid from matchrecord"
		. " where (matchstatus='notready') and tournament=$qturn and tournament_group=$qgroup order by $or";
	my $sth = &query($dbh,$q);
	my $nr = &numRows($sth);
	my $row = 0;
	...
	{
	my $q = "select player1,player2,outcome1,outcome2,player1_points,player2_points,admin,played,comment,comment1,comment2,matchid from matchrecord"
		. " where (matchstatus!='notready') and tournament=$qturn and tournament_group=$qgroup order by $or";
	my $sth = &query($dbh,$q);
	my $nr = &numRows($sth);
	#
...
{	my ($dbh,$tid,$matchtype,$admin) = @_;
	...
	{	my ($name,$type,$status,$comment,$sortkey) = &nextArrayRow($sth);	
		if($matchtype eq 'matchparticipant')
		{
		&show_matches_in_participant_group($dbh,$tid,$matchtype,$name,$type,$status,$comment,$sortkey,$admin);
		}
		else 
		{	
		&show_matches_in_group($dbh,$tid,$matchtype,$name,$type,$status,$comment,$sortkey,$admin);
		}
	}
...
{	my ($dbh,$tournamentid,$group,$sortkey,$admin)=@_;
	...
	while($firstex < $lastex)
	{
		...
		if($del) 
		{ my $q = "delete from matchparticipant where matchid=$qmatch";
		  &commandQuery($dbh,$q); 
		  #print "del $q<p>";
		}
		elsif($email)
		...
		{	my $pl = 0;
			while($pl<6)
			{
			my $nn = "player${pl}_${firstex}";
			my $pid = &param($nn);
			if($pid!=0)
			{
			my $stat = ($pid==-2) ? "'notready'" : "'ready'";
			my $qpl1 = $dbh->quote($pid);
		    my $q = "insert into matchparticipant set player=$qpl1,matchstatus=$stat where matchid=$qmatch";
			&commandQuery($dbh,$q);
			print "Q: $q<br>";
			}
			$pl++;
	...
	while($pn<=6)
	{
	my $pl1 = &param("player${pn}_$first");
	if($pl1!=0)
		{
		my $qpl1 = $dbh->quote($pl1);
		
		my $q = ($pl1 eq -2) 
				? "INSERT INTO matchparticipant SET player='-1', matchstatus='notready' ,tournament=$qtour,tournament_group=$qgroup"
				: "INSERT INTO matchparticipant SET player=$qpl1,tournament=$qtour,tournament_group=$qgroup";
		&commandQuery($dbh,$q);
		}
	...
	{	#print "Email match $qtour group $egroup<br>\n";
		my $q = "select matchid,player1,player2 from matchrecord where tournament=$qtour and tournament_group=$qgroup";
		my $sth = &query($dbh,$q);
		my $nr = &numRows($sth);
		while($nr-- > 0)
...
{	my ($dbh,$tournamentid,$group,$sortkey,$admin)=@_;
	...
	while($firstex < $lastex)
	{
		...
		{	my $stat = (($pl1==-2)||($pl2==-2)) ? "'notready'" : "'ready'";
		    my $q = "update matchrecord set player1=$qpl1,player2=$qpl2,matchstatus=$stat where matchid=$qmatch";
			&commandQuery($dbh,$q);
		}
		$firstex++;
	}
	...
	if($pl1 && $pl2)
		...
		my $q = ($pl2 eq -2) 
				? "INSERT INTO matchrecord SET player1=$qpl1,player2='-1', matchstatus='notready' ,tournament=$qtour,tournament_group=$qgroup"
				: "INSERT INTO matchrecord SET player1=$qpl1,player2=$qpl2,tournament=$qtour,tournament_group=$qgroup";
		&commandQuery($dbh,$q);
		}
	$first++;
	}}
...
sub matchtypeForVariation()
{	my ($dbh,$variation) = @_;
	my $q = "select max_players from variation where name=$variation";
	#print "Q: $q<p>";
	my $sth = &query($dbh,$q);
	my $nr = &numRows($sth);
	my $type = 'matchrecord';
	if($nr==1)
	{
		...
		if($players>2) { $type = 'matchparticipant'; }
	}
	&finishQuery($sth);
	return($type);	# default
}
...
{	my ($dbh,$tournamentid,$admin) = @_;
	...
	my $variation = $dbh->quote($variation);
	my $matchtype = &matchtypeForVariation($dbh,$variation);
	my $qmatchtype = $dbh->quote($matchtype);
...
sub show_edit_participant_match()
{	print "show_edit_participant_match not implemented<p>"
}
sub show_edit_match()
{	my ($dbh,$matchid,$fromname,$admin) = @_;
	...
	my $q = "select matchgroup.status,player1,player2,player1_points,player2_points,tournament,outcome1,outcome2,admin,played,"
		. " matchrecord.comment,matchrecord.comment1,matchrecord.comment2,tournament_group"
...
{	my ($dbh,$matchid,$msg) = @_;
	...
	if($matchid>0)
	{
	my $q = "select player_name,e_mail,language from matchrecord "
				. " left join players on ((matchrecord.player1 = players.uid) or (matchrecord.player2 = players.uid))"
				. " where matchrecord.matchid = '$matchid' " ;
	my $sth = &query($dbh,$q);
	my $nr = &numRows($sth);
	my $lan = $lib'language;
	while($nr-- > 0)
...
{	my ($dbh,$matchid) = @_;
	...
	{
	my $qm = $dbh->quote($matchid);
	my $q = "update matchrecord set matchstatus='ready',player2='-1' where matchid = $qm";
	&commandQuery($dbh,$q);
	}
...
sub standings_table()
{
	...
	my $q = 
		...
		. " from participant left join matchrecord"
		. " on matchrecord.tournament = $qtid and matchrecord.player1=participant.pid"
		. " left join matchgroup on matchgroup.type='swiss' and matchgroup.uid = $qtid and matchgroup.name = matchrecord.tournament_group "
		...
		. " from participant left join matchrecord"
		. " on matchrecord.tournament = $qtid and matchrecord.player2=participant.pid"
		. " left join matchgroup on matchgroup.type='swiss' and matchgroup.uid = $qtid and matchgroup.name = matchrecord.tournament_group "
		. " left join players on players.uid = pid"
	...
	{print "<b>Incorrect admin password<b><p>\n";
	  ...
	  if($operation eq '')
	   ...
	   {	my $sortkey = param('sortkey');
   	   ...
   	   elsif ($operation eq 'createparticipantmatch')
	   {	my $sortkey = param('sortkey');
			&createParticipantMatches($dbh,$tournamentid,$group,$sortkey,$admin);
		    &show_tournaments($dbh,$uid,$fromname,$passwd,0,$tournamentid,$admin);
	   }
  	   elsif ($operation eq 'doeditParticipantMatch')
	   {	my $matchid = param('matchid');
			...
			if($editok)
			{
			    if(!$closed)
			    {
				&do_edit_participant_match($dbh,$matchid,$editok,$email,$admin);
			    }
			}
			elsif($fromname && $passwd && !$admin)
			{	print "Sorry, wrong password<p>\n";
			}
			&show_edit_participant_match($dbh,$matchid,$fromname,$admin);
	   }
	   ...
	   elsif ($operation eq 'editparticipantmatch')
	   {	my $matchid = param('matchid');
			&show_edit_participant_match($dbh,$matchid,$fromname,$admin);
	   }
