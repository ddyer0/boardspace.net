...
# cgi-lib@pobox.com
# $Id: cgi-lib.pl,v 1.1.2.1 2004/03/30 05:22:29 ddyer Exp $
#
...
$cgi_lib'version = sprintf("%d.%02d", q$Revision: 1.1.2.1 $ =~ /(\d+)\.(\d+)/);
    ...
    } elsif ($meth eq 'POST') 
{
        if (($got = read(STDIN, $in, $len) != $len))
		  {$errflag="Short Read(1): wanted $len, got $got\n";};
   		 }else
     	{
   ...
   PART: # find each part of the multi-part while reading data
    while (1) {
      ...
      die "Short Read(2): wanted $amt, got $got\n" if $errflag;
      $left -= $amt;
     ...
     BODY: 
	  ...
	  &CgiDie("cgi-lib.pl: reached end of input while seeking boundary " .
        ...
        $errflag = (($got = read(STDIN, $buf, $amt, length($buf))) != $amt);
	die "Short Read(3): wanted $amt, got $got\n" if $errflag;
        $left -= $amt;
      }
      ...
      $amt = ($left > $bufsize+$maxbound-length($buf) 
	      ? $bufsize+$maxbound-length($buf) : $left);
      $errflag = (($got = read(STDIN, $buf, $amt, length($buf))) != $amt);
      die "Short Read(4): wanted $amt, got $got\n" if $errflag;
      $left -= $amt;
     ...
     HEAD:
	  ...
	  &CgiDie("cgi-lib: reached end of input while seeking end of " .
        ...
        $amt = ($left > $bufsize ? $bufsize : $left); #$maxbound==length($buf);
        $errflag = (($got = read(STDIN, $buf, $amt, length($buf))) != $amt);
        die "Short Read(5): wanted $amt, got $got\n" if $errflag;
        $left -= $amt;
      }
      ...
      if ($writefiles && defined $fname && ($fname ne "")) {
	  #
	  #a bit of customization, try to preserve the file extension
	  #or if none add one appropriate to the image type
	  #
	  my @parts = split(/\./,$fname);
	  my $type = $parts[$#parts];
	  if($type eq "") 
	  { if(($ctype eq "image/pjpeg")||($ctype eq "image/jpeg")) { $type = "jpg";  }
	    elsif ($ctype eq "image/gif") { $type = "gif"; }
	  }
	  if($type ne "") { $type = ".".$type; }
        $ser++;
	$fn = $writefiles . ".$$-$ser" . $type;
