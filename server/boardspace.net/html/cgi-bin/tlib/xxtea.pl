...
#\**********************************************************/
# ddyer 11/2020
# modified slightly for boardspace, to convert string encryption keys
# to 16 bytes using md5
# and to not export generic "encrypt" "decrypt" to avoid breaking
# older code that uses plain tea.
#
...
sub _long2str {
    ...
    if ($w) {
        my $m = $v->[$len - 1];
        if (($m < $n - 3) || ($m > $n)) {
	    print "oops main length $n embedded length $m\n";
            return 0;
        }
        $n = $m;
    }
...
sub xxtea_decrypt {
   ...
   # this is a bit of customization which makes this xxtea not quite as delivered
   # convert the string passphrase to 16 bytes using md5 and our special sugar
   # this algorithm has to agree with the java implementation
   sub fixkey
   {	my ($key) = @_;
	my $v = &md5("teasugar$key");
	my @val = unpack("c*",$v);
	return $v; 
   }
