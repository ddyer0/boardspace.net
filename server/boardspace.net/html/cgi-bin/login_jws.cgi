...
{ my ($dbh,$fav,$pname,$uid,$languageName,$uidrank,$haspicture,$country,$latitude,$logitude,
 ...
 my $java = param('java');
  ...
  &finishQuery($sth);
   ...
   my $port = $test ? $::test_server_port : $::game_server_port;
   print <<End;
	<applet codebase=http://$ENV{'HTTP_HOST'}/$::java_dir/$use_class_dir archive=$jarclass code=$rootclass width=1 height=1>
	<!-- *** -->
...
{ 
   ...
   my %location = &getLocation($ENV{'REMOTE_ADDR'});
	...
	if(($frame eq "yes") || ($frame eq "no"))
	{
       ...
       {my $now = time();
        my $jws = $forjws ? "&jws=1" : "";
       	my $some_args = "&pname=$pname&language=$language$jws";
        ...
        print "<frame name=applet src=\"http://$ENV{'HTTP_HOST'}$ENV{'SCRIPT_NAME'}?frame=applet&uid=$uid$some_args\" scrolling=no noresize>\n" ;
	#
    ...
    &print_applet($dbh,$fav,$pname,$uid,$languageName,$uidrank,$haspicture,$country,
          $latitude,$logitude,$pclass,$games_played,$timec,$bannercookie,$bannermode);
    }
   ...
   elsif ($frame eq "applet")
	{
	 if($forjws)
	 {
	 print "application/x-java-jnlp-file\n\n";
