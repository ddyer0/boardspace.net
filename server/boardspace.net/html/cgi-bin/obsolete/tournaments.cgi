...
{	my ($dbh,$pname,$passwd) = @_;
	my ($ok,$bounce) = &logon($dbh,$pname,$passwd);
	return($ok);
}
...
sub show_getadminpassword()
{
	...
	print "</td><td>";
	print "\n<input type=password value='' name=admin_user_password><br>";
	print "</td><td>";
	print "\n<input type=submit>";
	print "</td></tr></table>";
	print("\n</form>");
...
{	my ($dbh,$admin,$closed,$row,$matchid,$matchplayerid,$pn,$pl1,$name1,@ids) = @_;
	...
	print "<a href=$ENV{'SCRIPT_NAME'}?operation=editmatch&fromname=$name1&t2admin=$admin&matchid=$matchid>edit</a>";	
	print "<input type=hidden name='player${pn}_$row' value=$qpl1>\n";
	print "<input type=hidden name='mpid${pn}_$row' value = $qm>\n";
	print "</td>";
	}	
	else
	{
...
{	my ($dbh,$tournamentid,$group,$type,$status,$mcomment,$sortkey,$admin) = @_;
	...
	if($admin)
	{
	print "<tr><td colspan=$matchplayers align=center><b>"
		. &trans("Who")
		. "</b></td><td><b>"
		. &trans("Result")
		. "</b></td><td><b>"
	...
	{	
		...
		print "<br>email <input type=checkbox name=email_$nrows>\n";
		print "<input type=hidden name=match_$nrows value='$matchid'>";
		print "<a href=$ENV{'SCRIPT_NAME'}?operation=editmatch&t2admin=$admin&matchid=$matchid>edit</a><br>";	
		print "Match $matchid";
		print "</td>";
		}	    
	}
	...
	if($pl1!=0)
		...
		if($change eq '' || ($played && ($played>$change))) { $change = $played; }
			...
			if($player1_points!='') { $points .= "$name1 : $player1_points<br>\n"; }
...
{	my ($dbh,$tournamentid,$group,$sortkey,$admin)=@_;
	...
	while($firstex < $lastex)
	{
		...
		{	
			...
			{
			my $pid = &param("match${pn}_$firstex");
			if($pid)
			{
			my $frompl = &param("player${pn}_$firstex");
			if($frompl!=$pid)
			{
			my $fromuid = &param("mpid${pn}_$firstex");
			my $qpid = $dbh->quote($pid);
			my $qfrompl = $dbh->quote($frompl);
			my $quid = $dbh->quote($fromuid);
			my $q = "update matchparticipant set player=$qpid where player=$qfrompl and uid=$quid and tournament=$qtour";
			&commandQuery($dbh,$q);
			}}}		
		}
		$firstex++;
...
{	my ($dbh,$matchid,$fromname,$admin) = @_;
	...
	{	print "<table border=1>";
		...
		my $p1namelink = "<a href=\"javascript:editlink('$p2name',0)\">$p2name</a>&nbsp;$tz2 ";
   ...
   if(&allow_ip_access($dbh,$ENV{'REMOTE_ADDR'})>=0)
	...
	if(($operation eq '') || ($operation eq 'showtournament'))
	   ...
	   elsif ($operation eq 'doeditmatch')
	   {
			...
			my $editok = ($admin&&!$fromname) ? -2 : &logonOk($dbh,$fromname,$passwd);
