 ...
 *
 *   $Id: usercp_register.php,v 1.3 2007/01/13 05:29:47 ddyer Exp $
 *
 *
 ***************************************************************************/
...
function show_coppa()
{
	...
	$template->assign_vars(array(
		...
		"U_AGREE_OVER13" => append_sid("profile.$phpEx?mode=register&amp;IaGRee1=true"),
		"U_AGREE_UNDER13" => append_sid("profile.$phpEx?mode=register&amp;IaGRee1=true&amp;coppa=true"))
	);
...
if ( $mode == 'register' && !isset($HTTP_POST_VARS['IaGRee1']) && !isset($HTTP_GET_VARS['IaGRee1']) )
{
...
$secretkey = "abaords";
$authkey = md5("bakey" . $secretkey . "bavar");
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if((!isset($HTTP_POST_VARS[$authkey])
                || $HTTP_POST_VARS[$authkey] != $authval
                || !isset($HTTP_POST_VARS[$timekey])
                || ($HTTP_POST_VARS[$timekey] ^ $timepad) > time() - 15))
	{	$error=TRUE;
		$error_message = "Hmm. Not sure. Please try again";
	}
	...
	if($icq != "")
	  { $error = TRUE;
	    $error_message = "leave icq blank";
	  }
	if($website != "")
	  {
	    $error = TRUE;
	    $error_message = "leave website blank";
	  }
	...
	else if ( $mode == 'register' )
	{
		...
		{
			$error = TRUE;
			$error_msg .= "Registration through this interface is disabled.  Your boardspace.net registration is all you need.";
		}
	}
...
else
{
	...
	$s_hidden_fields = '<input type="hidden" name="mode" value="'
	  . $mode
	  . '" />
        <input type="hidden" name="IaGRee1" value="true" />
	...
	$s_hidden_fields .= '<input type="hidden" name="' . $authkey . '" value="' . $authval . '">';
	$s_hidden_fields .= '<input type="hidden" name="' . $timekey . '" value="' . $timeval . '">';
