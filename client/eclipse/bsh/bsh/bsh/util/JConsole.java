...
public class JConsole extends JScrollPane
	...
	MouseListener, MouseMotionListener, ActionListener, PropertyChangeListener 
{	private Interpreter interpreter = null;
	public void setInterpreter(Interpreter p) { interpreter = p; }
	...
	// bottom level, really print a string
	public void actualPrint(String s)
	{
		append(s);	// taps the toString method
		resetCommandStart();
		int end = cmdStart;
		text.setCaretPosition(cmdStart);
	}
	...
	public void print(final String o) {
	...
	public void print(final String o, final Font font, final Color color) {
		invokeAndWait(new Runnable() {
			public void run() {
    ...
    public void	actionPerformed(ActionEvent event) {
		...
		else handleRightClickAction(cmd);
    }
	...
	*/
	...
	/** support for live objects */
	//
	// RegisteredObject remembers the text position associated with a non-string object
	//
	class RegisteredObject
	{
		Object target;		// the underlying object we're remembering
		int start;			// start position of its text
		int end;			// end position of its text
		// constructor
		RegisteredObject(Object o,int s,int e)
		{
			target = o;
			start = s;
			end = e;
		...
		// pretty please
		public String toString() { return("<ro "+start+"-"+end+" "+target.toString()+">"); }
	}
	
	// a simple vector of all the registered objects.  Ought to be elaborated into a tree
	// searchable by position or something like that.
	Vector<RegisteredObject>registeredObjects = new Vector<RegisteredObject>();
	//
	// register an object
	public void registerObject(Object o,int start,int end)
	{	if(o instanceof String) {}	// don't register strings
		else 
		{RegisteredObject p = new RegisteredObject(o,start,end);
		 registeredObjects.addElement(p);
		}
	}
	// find a registered object at a given text position.
	// later, should consider nested, overlapping and embedded objects
	// and find the most specific 
	public RegisteredObject findSelectedObject(int pos)
	{	RegisteredObject found = null;
		
		for(int i=registeredObjects.size()-1; i>=0;i--)
		{	RegisteredObject ro = registeredObjects.elementAt(i);
			if((ro.start<=pos) && (pos<ro.end))
			{	
				found = ro;		// found, but keep looking
			}
		}
		return(found);
	}
	// print an object and remember its position
	public void actualPrint(final Object o) 
	{	if(o instanceof String) { actualPrint((String)o); }
		else 
		{
		int start = cmdStart;
		// this is where we can get a lot fancier, adding an optional "pretty print" protocol
		// which cooperates with this printer.
		actualPrint("{");
		actualPrint(String.valueOf(o));	// taps the toString method
		actualPrint("}");
		int end = cmdStart;
		registerObject(o,start,end); 
		}
	}


	public void mouseDragged(MouseEvent e) {
		// here we could be like mouseMoved but drag
		// the object to an inspector pane
	}

	private JPopupMenu rightClickMenu = null;
	private String INSPECT = "Inspect";
	private String DESCRIBE = "Describe";
	private Object rightClickTarget = null;
	
	public void doRightClick(Object ob,MouseEvent event)
	{
		
		// create popup	menu
		rightClickMenu = new JPopupMenu("Object Right Click Menu");
		rightClickTarget = ob;
		rightClickMenu.add(new JMenuItem(INSPECT)).addActionListener(this);
		rightClickMenu.add(new JMenuItem(DESCRIBE)).addActionListener(this);
		rightClickMenu.show(
				(Component)event.getSource(), event.getX(), event.getY());
	}
	public void handleRightClickAction(String cmd)
	{
		if(rightClickTarget!=null)
		{
			if(cmd.equals(INSPECT))
			{	Inspector.inspect(rightClickTarget);
			}
			else if (cmd.equals(DESCRIBE))
			{	// print the object again at the command line position
		  		if(interpreter!=null) { interpreter.setu("$_", rightClickTarget); }
		  		print(rightClickTarget);
		  		println();
			}
			}
	}
    public void	mouseClicked(MouseEvent	e) {
		...
		if(pos>0)
		{ RegisteredObject ob = findSelectedObject(pos);
			// TODO: click right for a menu of operations - describe etc.
		  if(ob!=null)
		  {	
			  switch(e.getButton())
			  {
			  default:
			  	{
			  		// print the object again at the command line position
			  		if(interpreter!=null) { interpreter.setu("$_", ob.target); }
			  		print(ob.target);
			  		println();
			  	}
			  	break;
	...
	public void mouseMoved(MouseEvent e) {
		...
		{ RegisteredObject ob = findSelectedObject(pos);
		  if(ob!=null)
		  {
			  // TODO: visualize the object so the user knows what he is pointing at
		  }
		}
	}
