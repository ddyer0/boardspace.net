 ...
 *
 */
public class ProteusPlay extends commonRobot<ProteusBoard> implements Runnable, ProteusConstants,
    RobotProtocol
{   
	...
	private RepeatedPositions repeats = null;
    /* constructor */
    public ProteusPlay()
    {
    }
 ...
 */
    ...
    {	ProteusMovespec mm = (ProteusMovespec)m;
    	boardSearchLevel--;
    	board.UnExecute(mm);
     }
 ...
 */
    ...
    {   return(board.GetListOfMoves(repeats));
    }
 ...
 * get a random move by selecting a random one from the full list.
 ...
 {	//return(board.Get_Random_Proteus_Move(rand));
 	...
 	try {
       if (board.DoneState())
        { // avoid problems with gameover by just supplying a done
            move = new ProteusMovespec(MOVE_DONE, board.whoseTurn);
        }
        else 
        {
        ...
        double randomn = RANDOMIZE && 
        					DUMBOT
        						? ((board.moveNumber <= 25) ? 0.3-0.3*board.moveNumber/25 : 0.0)
        						: ((board.moveNumber<8) ? 0.1*board.moveNumber/10 : 0.0);
