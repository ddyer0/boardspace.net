 	...
 	try {
       if (board.DoneState())
        { // avoid problems with gameover by just supplying a done
            move = new Rajmovespec("Done", board.whoseTurn);
        }
        else if(board.getState()==RajState.SELECT_PRIZE_STATE)
        {	// we don't really get to select, so skip the hund
        	return(new Rajmovespec("Select -1",board.whoseTurn));
        }
        else {
