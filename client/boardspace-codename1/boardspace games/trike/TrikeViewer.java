 ...
 *  
*/
public class TrikeViewer extends CCanvas<TrikeCell,TrikeBoard> implements TrikeConstants, GameLayoutClient, PlacementProvider
{		// move commands, actions encoded by movespecs.  Values chosen so these
    ...
    // private state
    private TrikeBoard bb = null; //the board from which we are displaying
    ...
    private Toggle eyeRect = new Toggle(this,"eye",
 			StockArt.NoEye,TrikeId.ToggleEye,NoeyeExplanation,
 			StockArt.Eye,TrikeId.ToggleEye,EyeExplanation
 			);
 ...
 */
    ...
    {	TrikeChip.preloadImages(loader,ImageDir);	// load the images used by stones
    ...
    {	
        ...
        // for another game.
        bb = new TrikeBoard(type,players_in_game,randomKey,getStartingColorMap(),TrikeBoard.REVISION);
        //
 ...
 //   }
    ...
    {	int player = pl.boardIndex;
        ...
        { // draw a random pile of chips.  It's just for effect
            ...
            TrikeChip chip = gb.getPlayerChip(player);
            TrikeCell cell = gb.getPlayerCell(player);
            int nc = 20;	
            // draw 20 chips
    ...
    * a game piece, it is really whatever is associated with b.movingObject()
    ...
    { 
     TrikeChip.backgroundTile.image.tileImage(gc, fullRect);   
      drawFixedBoard(gc);
     }
    
    // land here after rotating the board drawing context if appropriate
    public void drawFixedBoard(Graphics gc,Rectangle brect)
    {	TrikeBoard gb = disB(gc);
        boolean reviewBackground = reviewMode()&&!mutable_game_record;
        if(reviewBackground)
        {	 
         TrikeChip.backgroundReviewTile.image.tileImage(gc,brect);   
        }
          ...
          for(Enumeration<TrikeCell>cells = gb.getIterator(Itype.TBRL); cells.hasMoreElements(); )
          { //where we draw the grid
        	  TrikeCell cell = cells.nextElement();
     ...
     */
    public void drawBoardElements(Graphics gc, TrikeBoard gb, Rectangle brect, HitPoint highlight)
    {
        //
...
*/
    ...
    {  
       ...
       // draw the board control buttons 
        ...
        drawPlayerStuff(gc,(state==TrikeState.Puzzle),buttonSelect,HighlightColor,rackBackGroundColor);
  
 
        // draw the avatars
        standardGameMessage(gc,messageRotation,
        					// note that gameOverMessage() is also put into the game record
            				state==TrikeState.Gameover?gameOverMessage(gb):s.get(state.description()),
 ...
 */
    ...
    {
        return (new Trikemovespec(st, pl));
    }
/**
 ...
 */
    ...
    {
        if (hp.hitCode instanceof TrikeId)// not dragging anything yet, so maybe start
        {
        TrikeId hitObject =  (TrikeId)hp.hitCode;
 	    switch(hitObject)
	    {
	    default: break;
 ...
 * not setting the values when the gc is null.
    ...
    public void StopDragging(HitPoint hp)
    {
        CellId id = hp.hitCode;
       	if(!(id instanceof TrikeId))  {   missedOneClick = performStandardActions(hp,missedOneClick);   }
        else {
        missedOneClick = false;
        TrikeId hitCode = (TrikeId)id;
        
        // if direct drawing, hp.hitObject is a cell from a copy of the board
        TrikeCell hitObject = bb.getCell(hitCell(hp));
		TrikeState state = bb.getState();
        switch (hitCode)
        {
        default:
