 ...
 *
 */
public class MajoritiesPlay extends commonRobot<MajoritiesBoard> implements Runnable, 
    RobotProtocol
    {
 ...
 */
    ...
    {	MajoritiesMovespec mm = (MajoritiesMovespec)m;
        board.UnExecute(mm);
        boardSearchLevel--;
    }
 ...
 */
 public void InitRobot(ViewerProtocol newParam, ExtendedHashtable info, BoardProtocol gboard,
        String evaluator, int strategy)
    {
        ...
        GameBoard = (MajoritiesBoard) gboard;
        ...
        case WEAKBOT_LEVEL: WEAKBOT = true;
			//$FALL-THROUGH$
		case DUMBOT_LEVEL: timePerMove = 5; break;
        case SMARTBOT_LEVEL: timePerMove = 15;  break;
        case BESTBOT_LEVEL: timePerMove = 30;   break;
        case MONTEBOT_LEVEL:   break;
        }
    }
 ...
 * get a random move by selecting a random one from the full list.
 ...
 {	return(board.Get_Random_Majorities_Move(rand));
 }
 
 // this is the monte carlo robot, which for majorities is much better then the alpha-beta robot
 	...
 	try {
       if (board.DoneState())
        { // avoid problems with gameover by just supplying a done
            move = new MajoritiesMovespec("Done", board.whoseTurn);
        }
        else 
        {
        ...
        monte_search_state.timePerMove = timePerMove;		// 5 or 10 seconds per move
        ...
        monte_search_state.final_depth = 9999;		// not needed for majorities which is always finite
        monte_search_state.node_expansion_rate = 1.0;
        monte_search_state.randomize_uct_children = true;     
        // we depend on blitz evaluation because the lineOwner information is not maintained on unwind
        monte_search_state.blitz = true;
        monte_search_state.maxThreads = DEPLOY_THREADS;
        monte_search_state.random_moves_per_second =     WEAKBOT ? 5000 : 300000;		// 
