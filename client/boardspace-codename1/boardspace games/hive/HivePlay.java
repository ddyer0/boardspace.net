 ...
 public commonMove DoAlphaBetaFullMove()
    {
        ...
        double time = 0.75;		// default time
        int addon = 0;
        switch(robotTime.kind)
        {
        case None: break;
        case PlusTime:
        	addon = robotTime.moveIncrement;      	
			//$FALL-THROUGH$
        default: 
        	int movesRemaining = Math.max(10, 20-board.moveNumber());
        	int remain = addon + viewer.timeRemaining(board.whoseTurn); 	// milliseconds
        	time = (remain/(double)movesRemaining)/(60*1000);		// time in minutes
        	
        	int secs = (int)(time*60);
        	G.print("Robot variable time "+secs+" seconds for "+movesRemaining);
        	break;
        }
        
        Search_Driver search_state = Setup_For_Search(depth,time,depth-1);	// 45 seconds
        try
        {
