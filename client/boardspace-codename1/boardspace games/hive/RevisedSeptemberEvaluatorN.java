 ...
 * 
 * This version is "N" in the ratings records, and designated as "bestbot"
 * 
 ...
 */
class RevisedSeptemberEvaluatorN extends DefaultEvaluator implements Evaluator
{	
	...
	private double gatedApproachCount = 0.25;			// benefit of a gate protecting the Q
	...
	{	
		...
		{ HiveCell c = queen.exitTo(direction);
		  ...
		  {	  // vacant space next to queen
			  ...
			  {
				  HiveCell d = c.exitTo(dir);
				  if(board.isGate(c,false,d,dir)) 
				  {
					  gatedApproach++;
				  }
			  }
		...
		credits -= gatedApproach*gatedApproachCount;
		}
		return(rawCount+credits);
	}	
/* this was the "L" version and variations 
	// return the number of adjacent cells owned by a player
	public double nOccupiedOrOwnedAdjacent(HiveGameBoard board,HiveCell queen, int targetPlayer)
	{	
		boolean myQueenAdj = false;
		int pillbugAdjacent = 0;
		int pillbugAdjacentSimple = 0;
		HiveId targetColor = board.playerColor(targetPlayer);
		HivePiece bug = queen.topChip();
		boolean enemyBeetle = bug.color!=targetColor;
		int gatedApproach = 0;
		int rawCount = 0;
		boolean pillbugThreatened = false;
		double credits = 0;
		  ...
		  if(h>0) 
		  	{
		  	  ...
		  	  if(top.color==targetColor) 
		  		  ...
		  		  if( ( (top.type==PieceType.PILLBUG)
		  			  ...
		  			  for(int i=0;i<6;i++) 
		  			  {	  
		  				  HiveCell adj = c.exitTo(i);
		  				  int adjHeight = adj.height();
		  				  if(adjHeight==0)
		  				  {
			  				  if(board.hex_cell_distance(adj,queen)>1)
			  				  {
			  					localCount++;		
			  				  }
			  			  }
		  				  ...
		  				  {
		  					  HivePiece adjTop = adj.topChip();
		  					  if((adjTop.color==targetColor)
		  						 && ((adjHeight>1 )
		  							 || (adjTop.type==PieceType.BEETLE)
		  							 || ((adjTop.type==PieceType.MOSQUITO) && adj.actingAsType(PieceType.BEETLE))))
		  					  {	pillbugThreatened = true; 
		  					  }
		  				  }
		  			  }
		  			  if(!pillbugThreatened) { pillbugAdjacent += localCount; }
		  			  }
		  	  	}
		  	  else if((h==1) 
		  			  && (top.type==PieceType.BEETLE 
		...
		if(myQueenAdj) { credits -= friendlyMobileAtQueenCount; }
		if(pillbugAdjacentSimple>0) 
			{	//give credit if the raw adjacent count is low, or if the accurate count is positive
			    if(rawCount<4 || pillbugAdjacent>0) { credits -= friendlyPillbugAtQueenCount; }
			    //credits -= friendlyPillbugAtQueenCount;
			}
