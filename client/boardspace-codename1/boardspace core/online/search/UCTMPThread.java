 ...
 *
 */
class UCTMPThread extends Thread implements Opcodes,UCTThread
{	boolean stop = false;		// request for the thread to stop
	...
	long pausedTime = 0;
	int scoredCount = 0;
	int unscoredCount = 0;		// unscored ought to be rare, and limited to multi-threaded games.
	//
	...
	// constructor
	UCTMPThread(UCTMoveSearcher m,RobotProtocol ro,String threadName)
	...
	{	
		...
		{	// if terminal is true, we encountered a marked terminal node in the
			...
			{	
				// this seems to correspond to the cases where the robot just spins
				// pointlessly.  we could do something more clever here.
				scorable=false;
			}
		}
		else if(gameOver)
		...
		master.root.update(0,1,master.alpha);
	...
	{	
		if(scoreMove!=null)
		{
		// under unsual circumstances when using multiple threads, moves
		...
		//
		// also, this is the key difference betweeb UCTSPThread and UCTMPThreads.  This version
		// insists that the score is multiplayer and maintained in the scoreMove object
		//
		boolean isScored = scoreMove.isUctScored();
		if(!isScored)
			{ if(scorable) { unscoredCount++; }}
			else 
			{ scoredCount++; }
		scorable &= isScored;
