...
{ // state shared with parent frame
   ...
   /** for games with a scaleable board, this is the conventional name for
    ...
    {	if(chatFramed) { return(0); }
    	...
    	int fs = FontManager.getFontSize(standardPlainFont());
...
{	Text msg = null;
	...
	GC.setColor(gc,cc);
	GC.setFont(gc,FontManager.getFont(standardBoldFont(),G.Height(stateRect)/2));
 ...
 */
    ...
    private void checkLayouts()
    {
    	...
    	for(int i=0;(i<10000)&&(layoutErrors==0);i++)
    	{
    		...
    		double fac = FontManager.adjustWindowFontSize(neww,newh);
    	...
    	if(layoutErrors==0) 
    		{ 
    		  double fac = FontManager.adjustWindowFontSize(w,h);
      	  	  adjustStandardFonts(fac*FontManager.defaultFontSize);
