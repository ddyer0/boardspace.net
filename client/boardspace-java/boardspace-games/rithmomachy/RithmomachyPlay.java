 ...
 *
 */
public class RithmomachyPlay extends commonRobot<RithmomachyBoard> implements Runnable, RithmomachyConstants,
    RobotProtocol
{   
 ...
 */
    ...
    {	RithmomachyMovespec mm = (RithmomachyMovespec)m;
    	boardSearchLevel--;
    	board.UnExecute(mm);
     }
 ...
 */
     ...
     * @return
     */
    private double ScoreForPlayer(RithmomachyBoard evboard,int player,boolean print)
    {	
 ...
 */
 ...
 public commonMove DoAlphaBetaFullMove()
    {
	 RithmomachyMovespec move = null;

     // it's important that the robot randomize the first few moves a little bit.
     int randomn = RANDOMIZE 
     				? ((board.moveNumber <= 4) ? (20 - board.moveNumber) : 0)
     				: 0;
     int depth = Math.min(MAX_DEPTH,randomn>0 ? 4 : MAX_DEPTH);	// search depth
     double dif = 0.5;		// stop randomizing if the value drops this much
     ...
     Search_Driver search_state = Setup_For_Search(depth, false);
        try
        {
            ...
            { // avoid problems with gameover by just supplying a done
                move = new RithmomachyMovespec("Done", board.whoseTurn);
            }
            ...
            if (move == null)
            {
                move = (RithmomachyMovespec) search_state.Find_Static_Best_Move(randomn,dif);
            }
        }
        finally
        {
