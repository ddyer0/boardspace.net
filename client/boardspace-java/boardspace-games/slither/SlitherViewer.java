...
{		// move commands, actions encoded by movespecs.  Values chosen so these
    ...
    static final String Slither_SGF = "slither"; // sgf game name

    // file names for jpeg images and masks
    static final String ImageDir = "/slither/images/";
    ...
    // private state
    private SlitherBoard bb = null; //the board from which we are displaying
 ...
 */
    ...
    {	SlitherChip.preloadImages(loader,ImageDir);	// load the images used by stones
		gameIcon = SlitherChip.Icon.image;
    }
    ...
    {	
        ...
        String type = info.getString(GameInfo.GAMETYPE, SlitherVariation.slither.name);
        // recommended procedure is to supply players and randomkey, even for games which
        // are current strictly 2 player and no-randomization.  It will make it easier when
        // later, some variant is created, or the game code base is re purposed as the basis
        // for another game.
        bb = new SlitherBoard(type,players_in_game,randomKey,getStartingColorMap(),SlitherBoard.REVISION);
        //
 ...
 //   }
    ...
    {	commonPlayer pl = getPlayerOrTemp(player);
    	...
    	Rectangle box =  pl.createRectangularPictureGroup(x+4*unitsize,y,2*unitsize/3);
    ...
    private void DrawChipPool(Graphics gc, Rectangle r, commonPlayer pl, HitPoint highlight,SlitherBoard gb)
    {	int player = pl.boardIndex;
        ...
        { // draw a random pile of chips.  It's just for effect
            ...
            SlitherChip chip = gb.getPlayerChip(player);
            SlitherCell cell = gb.getPlayerCell(player);
            int nc = 20;	
            // draw 20 chips
    ...
    * a game piece, it is really whatever is associated with b.movingObject()
    ...
    {	boolean reviewBackground = reviewMode()&&!mutable_game_record;
        ...
        if(reviewBackground)
        {	 
         SlitherChip.backgroundReviewTile.image.tileImage(gc,boardRect);   
        }
         
        scaled = SlitherChip.board_19.image.centerScaledImage(gc, boardRect,scaled);

        gb.DrawGrid(gc,boardRect,use_grid,Color.white,Color.black,Color.black,Color.black);
        int l = G.Left(brect);
        int t = G.Top(brect);
        SlitherCell A1 = gb.getCell('A',1);
        char lastc = (char)('A'+gb.ncols-1);
        SlitherCell Xx = gb.getCell(lastc,gb.nrows);
        int sz = gb.cellSize();
        int left = gb.cellToX(A1);
        int right = gb.cellToX(Xx);
        int top = gb.cellToY(A1);
        int bottom = gb.cellToY(Xx);
        GC.setColor(gc,new Color(0.2f,0.2f,0.2f));
        GC.fillRoundRect(gc,l+left,t+top-sz/2,right-left,sz/6,sz/6,sz/6);
        GC.fillRoundRect(gc,l+left,t+bottom+sz/10,right-left,sz/6,sz/6,sz/6);
        GC.setColor(gc,new Color(0.9f,0.9f,0.9f));
        GC.fillRoundRect(gc,l+left-3*sz/7,t+top-sz/8,sz/6,bottom-top,sz/6,sz/6);
        GC.fillRoundRect(gc,l+right+1*sz/4,t+top-sz/8,sz/6,bottom-top,sz/6,sz/6);
     ...
     */
    public void drawBoardElements(Graphics gc, SlitherBoard gb, Rectangle brect, HitPoint highlight)
    {
        //
...
*/
    ...
    {  
       ...
       boolean conf = (state==SlitherState.ConfirmSwap) ;
        ...
        drawPlayerStuff(gc,(state==SlitherState.Puzzle),buttonSelect,HighlightColor,rackBackGroundColor);
  
 
        // draw the avatars
        standardGameMessage(gc,messageRotation,
        					// note that gameOverMessage() is also put into the game record
            				state==SlitherState.Gameover?gameOverMessage(gb):s.get(state.description()),
            				state!=SlitherState.Puzzle,
            				gb.whoseTurn,
            				stateRect);
 ...
 */
    ...
    {
        return (new Slithermovespec(st, pl));
    }
/**
 ...
 */
    ...
    {
        if (hp.hitCode instanceof SlitherId)// not dragging anything yet, so maybe start
        {
        SlitherId hitObject =  (SlitherId)hp.hitCode;
 	    switch(hitObject)
	    {
	    default: break;
 	    ...
 	    case White:
	    	...
	    	break;
	    case BoardLocation:
	        SlitherCell hitCell = hitCell(hp);
 ...
 * not setting the values when the gc is null.
    ...
    public void StopDragging(HitPoint hp)
    {
        CellId id = hp.hitCode;
       	if(!(id instanceof SlitherId))  {   missedOneClick = performStandardActions(hp,missedOneClick);   }
        else {
        missedOneClick = false;
        SlitherId hitCode = (SlitherId)id;
        ...
        case White:
           if(bb.pickedObject!=null) 
			{//if we're dragging a black chip around, drop it.
            	PerformAndTransmit(G.concat("Drop ",bb.pickedObject.id));
			}
           break;
    ...
    private boolean setDisplayParameters(SlitherBoard gb,Rectangle r)
    {
 ...
 //    }
  ...
  //  }
    ...
    public String sgfGameType() { return(Slither_SGF); }	// this is the official SGF number assigned to the game
