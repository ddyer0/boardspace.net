 ...
 *
 */
public class TintasPlay extends commonRobot<TintasBoard> implements Runnable, TintasConstants,
    RobotProtocol
    {
 ...
 */
    ...
    {	Tintasmovespec mm = (Tintasmovespec)m;
        board.UnExecute(mm);
        boardSearchLevel--;
    }
...
{	
	 ...
	 {	int nChildren = parent.getNoOfChildren();
		 ...
		 if(node!=null && (node.getVisits()>=0))
		 {
			...
			if(winrate<-0.2)
				{ if(search.verbose>1)
					{
					G.print("Randomization skipped because we're losing already");
					}
					return(bestChild); 
				}
		 }
	 	}
		return(super.getBestWinrateMove(parent,r,search));
	 }
 // this is the monte carlo robot, which for tintas is much better then the alpha-beta robot
 ...
 {	commonMove move = null;
 	...
 	try {
        ...
        monte_search_state.win_randomization = RANDOMIZATION;		// a little bit of jitter because the values tend to be very close
        ...
        monte_search_state.blitz = false;			// blitz is typically 2/3 the speed of normal unwinds
        monte_search_state.sort_moves = false;
        monte_search_state.only_child_optimization = true;
        monte_search_state.dead_child_optimization = true;
        monte_search_state.simulationsPerNode = 1;
        monte_search_state.killHopelessChildrenShare = CHILD_SHARE;
        monte_search_state.final_depth = 9999;		// note needed for tintas which is always finite
