package crosswordle;
 ...
 // DrawRepRect to warn the user that repetitions have been seen.
	 ...
	 {	StringTokenizer tok = new StringTokenizer(solution);
		for(int row=nrows; row>0; row--)
		 {
			 for(int col = 0; col<ncols; col++)
			 {	CrosswordleCell c = getCell((char)('A'+col),row);
			 	String next = tok.nextToken();
			 	CrosswordleChip chip = CrosswordleChip.getLetter(next.charAt(0));
			 	c.reInit();
			 	c.addChip(chip);
			 }
		 }
	...
	public void scoreWord(String w)
	{	CrosswordleCell col = getCell('A',nrows);
		CrosswordleCell row = col;
		int len = w.length();
		if(len==nrows) { while(col!=null) { scoreWord(w,col,CELL_DOWN); col =col.exitTo(CELL_RIGHT); }}
		if(len==ncols) { while(row!=null) { scoreWord(w,row,CELL_RIGHT); row = row.exitTo(CELL_DOWN); }}
	}
	public void scoreWord(String word,CrosswordleCell from,int direction)
	{	CrosswordleCell c = from;
		for(int i=0,lim=word.length(); i<lim; i++)
		{
			...
			if(wchar==top) { c.color = LetterColor.Green; }
			else if(c.color==LetterColor.Blank){ 
				for(int j=0;j<word.length();j++)
				{
					CrosswordleChip target = CrosswordleChip.getLetter(word.charAt(j));
					if(target==top) { c.color = LetterColor.Yellow; break; }
				}				
			}
			c = c.exitTo(direction);
		}
