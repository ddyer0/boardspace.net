...
class WordStack extends OStack<Word>
{
	...
	public Word recordCandidate(String message,CrosswordleCell c,String s,int direction,int score,Entry e)
...
class Word implements StackIterator<Word>,CompareTo<Word>
{
	...
	CrosswordleCell seed;	// starting point
	...
	public CrosswordleCell lastLetter()
	{
		CrosswordleCell s = seed;
	...
	public Word(CrosswordleCell s, String n, int di)
	{
		seed = s;
		name = n;
...
class CrosswordleBoard extends rectBoard<CrosswordleCell> implements BoardProtocol
{	static int REVISION = 108;			// 100 represents the initial version of the game
	...
	int sweep_counter = 0;
	CrosswordleVariation variation = CrosswordleVariation.Crosswordle;
	...
	CrosswordleCell rack[][] = null;
	private CrosswordleCell mappedRack[][] = null;
	
	public CrosswordleCell[] getPlayerRack(int n)
	{
		if(rack!=null && rack.length>n) { return(rack[n]); }
		return(null);
	}
	public CrosswordleCell[] getPlayerMappedRack(int n)
	{
	...
	{
		for(CrosswordleCell r[] : rack) { if(rackIsEmpty(r)) { return(true); }}
		return(false);
	}
	private boolean rackIsEmpty(CrosswordleCell r[])
	{	for(CrosswordleCell c : r) { if(c.topChip()!=null) { return(false); }}
		return(true);
	}
	public boolean hiddenVisible[] = null;
	int score[] = null;
	CrosswordleCell drawPile = null;
	int chipsOnBoard = 0;
...
// DrawRepRect to warn the user that repetitions have been seen.
    ...
    private CrosswordleCell seedLocation = null;
	...
	{	return(new CrosswordleCell(CrosswordleId.BoardLocation,c,r,Geometry.Oct));
	}
    ...
    public CrosswordleBoard(String init,int players,long key,int map[],Dictionary di,int rev) // default constructor
    {
		...
		openRack = new boolean[MAX_PLAYERS];		// part of the user interface
      	...
      	Random r = new Random(2975564);
       	drawPile = new CrosswordleCell(r,CrosswordleId.DrawPile);
    ...
    public CrosswordleCell allPostCells = null;
    private CrosswordleCell bonusCells[] = null;
    
    public cell<?> postBoard[][] = null;
    @SuppressWarnings("unchecked")
	public CrosswordleCell getPostCell(char col,int row)
    {	int coln = col-'A';
    	if(coln>=0 && coln<postBoard.length)
    	{
    		cell<CrosswordleCell>[] crow =(cell<CrosswordleCell>[])postBoard[coln];
    		if(row>=1 && row<=crow.length) {
    			return((CrosswordleCell)crow[row-1]);
    		}
    	}
    	return(null);
    }
    ...
    {	Random r = new Random(6278843);
        ...
        for(CrosswordleCell c = allCells; c!=null; c=c.next) { c.isPostCell = true; }
        allPostCells = allCells;
        // create a grid for the tile cells
        allCells = null;
        initBoard(n,k);		 //this sets up the primary board and cross links
        allCells.setDigestChain(r);
        for(CrosswordleCell c = allCells; c!=null; c=c.next) { c.isTileCell = true; }
    ...
    {	int numberOfBonuses = variation.boardsize;
		...
		bonusCells = new CrosswordleCell[numberOfBonuses];
 		// position the bonus squares randomly
 		for(CrosswordleCell c = allPostCells; c!=null; c=c.next) { st.push(c); c.addChip(CrosswordleChip.Post); }
 		st.shuffle(r);
 		for(int i=0;i<numberOfBonuses;i++)
			{	CrosswordleCell c = st.pop();
				int v = r.nextInt(10);
				c.removeTop();
				bonusCells[i]=c;
				switch(v)
				...
				case 0:
				case 1:
				case 2: c.addChip(CrosswordleChip.DoubleLetterGreen);
						break;
				case 3:
				case 4:
						c.addChip(CrosswordleChip.TripleLetterYellow);
						break;
				case 5:
				case 6:
				case 7:
						c.addChip(CrosswordleChip.DoubleWordBlue);
						break;
				case 8:
				case 9: c.addChip(CrosswordleChip.TripleWordRed);
						break;
    ...
    {	randomKey = key;
		...
		case Crosswordle:
 		...
 		seedLocation.addChip(CrosswordleChip.A);
    	...
    	drawPile.reInit();
    	for(CrosswordleChip c : CrosswordleChip.letters)
    	{
    		drawPile.addChip(c);
    	}
    	drawPile.shuffle(r);
 		...
 		{
    	rack = new CrosswordleCell[players][racks];
       	mappedRack = new CrosswordleCell[players][rackSize+2];
        
    	for(int i=0;i<players;i++)
    	{	CrosswordleCell prack[] = rack[i];
    		for(int j = 0;j<prack.length;j++)
    			{ CrosswordleCell c = rack[i][j] = new CrosswordleCell(CrosswordleId.Rack,(char)('A'+i),j,Geometry.Standalone);
    			  c.onBoard = false;
    			}
    		CrosswordleCell mrack[] = mappedRack[i];
    		for(int j = 0;j<mrack.length; j++)
    		{
  			  mrack[j] = new CrosswordleCell(CrosswordleId.RackMap,(char)('A'+i),j,Geometry.Standalone);
    ...
    public boolean dropIsHorizontal()
    {
    	...
    	CrosswordleCell c1 = droppedDestStack.top();
    	CrosswordleCell c0 = droppedDestStack.elementAt(0);
    ...
    {	int sz = droppedDestStack.size();
    	if(sz<=1) { return(true); }
    	CrosswordleCell root = droppedDestStack.elementAt(0);
    	boolean fixedrow = true;
    	boolean fixedcol = true;
    	for(int lim=droppedDestStack.size()-1; lim>=1; lim--)
    		{	CrosswordleCell c = droppedDestStack.elementAt(lim);
   ...
   /**
    ...
    {	pickedSourceStack.push(c);
        ...
        case Rack:
        	...
        	for(int i=0;i<map.length;i++) { if(map[i]==c.row) { map[i]=-1; break; }} 
          	CrosswordleChip po = c.removeTop();
    ...
    private void setNextStateAfterDrop(CrosswordleCell c,CrosswordleChip ch,replayMode replay)
    {
        ...
        case ResolveBlank:
        	...
        	else if(ch==CrosswordleChip.Blank && c.onBoard) { setState(SprintState.ResolveBlank); }
    ...
    {	
    	for(CrosswordleCell c : rack[who]) { if(c.topChip()!=null) { return(false); }}
    	return(true);
    }
    ...
    {	CrosswordleCell myrack[] = rack[whoseTurn];
    	CrosswordleCell mapped[] = mappedRack[whoseTurn];
    	int mymap[] = rackMap[whoseTurn];
    	int n=0;
    	validateMap(whoseTurn);

    	for(CrosswordleCell c : myrack) { if(c.topChip()!=null) { n++; }}
    	for(CrosswordleCell c : myrack)
    		{ if(drawPile.height()==0 || n==rackSize) break;
    		  if(c.topChip()==null)
    			  {
    			  CrosswordleChip newchip = drawPile.removeTop();
    			  c.addChip(newchip);
    			  n++;
    			  if(replay!=replayMode.Replay) 
    			  {	  CrosswordleCell cd = mapped[c.row];
   ...
   private synchronized boolean validate(boolean andScore)
    ...
    // return some duplicate word if there are any.
   	...
   	// 
    private String collectWord(CrosswordleCell from,int direction)
    {
    	builder.setLength(0);
    	int n=0;
    	isNew = false;
    ...
    // just once.
    private boolean addWordBonus(CrosswordleCell c)
    {
    ...
    CrosswordleCell wordBonus[] = new CrosswordleCell[4];
    // score a word on the board.  wordMultiplier and wordBonus are
    // used as scratch variables during the scoring.
    private int scoreWord(CrosswordleCell from,int direction)
    {	int score = 0;
    	builder.setLength(0);
    	wordMultiplier = 1;
    	isNew = false;
    ...
    public int checkIllegalCrosswords(String word,int crossIndex,CrosswordleCell from,int inDirection,CrosswordleCell fromRack[])
    {	int extraScore = 0;
    	CrosswordleCell c = from;
    	...
    	for(int idx = 0,len=word.length(); idx<len; idx++,c = c.exitTo(inDirection))
    	{
    		if(idx!=crossIndex)
    		{
    		 	...
    		 	{	
    		 		...
    		 		CrosswordleCell head = c;
    		 		CrosswordleCell prev = c.exitTo(revDir);
    		 		CrosswordleCell next = c.exitTo(direction);
    		 		int nchars = 0;
    		 		if(prev!=null) 
    		 		{
    		 			CrosswordleChip ptop = prev.topChip();
    		 			if(ptop!=null)
    		 			{
    		 				builder.setLength(0);
    		 				do 
    		 		...
    		 		if(next!=null)
    		 			{
    		 			CrosswordleChip ntop = next.topChip();
    		 			if(ntop!=null)
    		 				{
    ...
    private int scoreLetter(CrosswordleCell c,CrosswordleChip top)
    {	int letterScore = top.value;
		for(int dx = -1; dx<=0; dx++)
		{ for(int dy = -1; dy<=0; dy++)
		{	CrosswordleCell post = getPostCell((char)(c.col+dx),c.row+dy);
			if(post!=null)
			{
				CrosswordleChip bonus = post.topChip();
				if(bonus!=null)
				{
					if(bonus==CrosswordleChip.DoubleLetterGreen) { letterScore *=2; }
					else if(bonus==CrosswordleChip.TripleLetterYellow) { letterScore *=3; }
					else if(bonus==CrosswordleChip.DoubleWordBlue) 
						{ if(addWordBonus(post))
							{wordMultiplier *=2; 
							}
						}
					else if(bonus==CrosswordleChip.TripleWordRed) 
						{ if(addWordBonus(post))
							{wordMultiplier *=3; 
							}
						}
					else if(bonus==CrosswordleChip.Post) {} 
    ...
    private int validateFrom(CrosswordleCell c)
    {
    	...
    	{	CrosswordleCell next = c.exitTo(dir);
     		CrosswordleChip top = next==null ? null : next.topChip();
    ...
    private void doDone(replayMode replay)
    {
        ...
        {
        	for(CrosswordleCell c : rack[whoseTurn])
        	{	CrosswordleChip top = c.topChip();
        		if(top!=null)
	...
	private void placeWord(CrosswordleCell from,CrosswordleCell rack[],String word,int direction,replayMode replay)
	{	CrosswordleCell c = from;
		...
		{	CrosswordleChip top = c.topChip();
			...
			CrosswordleCell placeFrom= findChar(rack,ch);
			pickObject(placeFrom);
			if(pickedObject==CrosswordleChip.Blank) {
				pickedObject = CrosswordleChip.assignedBlanks[ch-'a'];
			}
			dropObject(c);
			if(replay!=replayMode.Replay)
				{
	...
	public void dropAndSlide(int who,CrosswordleCell from,int moving0,int pick0,int dest0,replayMode replay)
	{	// the map is invalid when we enter, because we've dropped a tile
		// and haven't mapped it yet.
		CrosswordleCell rcells[] = mappedRack[who];
    	int pick = pick0;
    	int dest = dest0;
    	int moving = moving0;
    	int map[] = rackMap[who];
    ...
    {	CrosswordleMovespec m = (CrosswordleMovespec)mm;
        ...
        case MOVE_PLAYWORD:
        	{
        	CrosswordleCell c = getCell(m.to_col,m.to_row);
        	placeWord(c,rack[whoseTurn],m.word,m.direction,replay);   
        	
        	CrosswordleCell r[] = rack[whoseTurn];
        ...
        case MOVE_MOVETILE:
        	{
           	validateMap(whoseTurn);
           	CrosswordleCell src = getCell(m.source,m.from_col,m.from_row);
        	CrosswordleCell dest = getCell(m.dest,m.to_col,m.to_row);
        	pickObject(src);
        	CrosswordleChip po = m.chip = pickedObject;
        	dropObject(dest);
           	validateMap(whoseTurn);
    ...
    public boolean LegalToHitChips(CrosswordleCell c)
    {
        switch (board_state)
        {
        default:
    ...
    public boolean LegalToHitBoard(CrosswordleCell c,boolean picked)
    ...
    {		int used = 0;
    		int mapped = 0;
    		int nmapped = 0;
    		CrosswordleCell prack[] = rack[forplayer];
    		for(CrosswordleCell c : prack)
    		{
 ...
 private CrosswordleCell canPlaceWord(String word,int firstIndex,CrosswordleCell from,int direction)
 {	int reverse = direction+CELL_HALF_TURN;
 	CrosswordleCell head = from;
 	int usedLetters = 0;
 	// check the previous letters for space
 	{
 	 	CrosswordleCell c = from.exitTo(reverse);
 	 	for(int idx = firstIndex-1; idx>=0; idx--)
 	 	{	if(c==null) { return(null); }		// can't fall off the edge
 		 	CrosswordleChip ch = c.topChip();
 	...
 	{
 	CrosswordleCell c = from.exitTo(direction);
 ...
 private int findLetter(CrosswordleCell fromRack[],char ch,boolean blank)
 {
	 for (int lim = fromRack.length-1; lim>=0; lim--)
	 {
		 CrosswordleCell c = fromRack[lim];
		 if(c.sweep_counter!=sweep_counter)
		 {	CrosswordleChip top = c.topChip();
 ...
 private int scoreWord(String word,CrosswordleCell from,int direction,CrosswordleCell fromRack[])
 {	
	int score = 0;
	sweep_counter++;
	wordMultiplier = 1;
 	...
 	{	
	 	...
	 	{	// we'll place this letter, so it scores
	 		...
	 		if(rackidx>=0)
	 			...
	 			score += scoreLetter(c,CrosswordleChip.getLetter(letter));
 ...
 private boolean canPlaceFromRack(String targetWord,CrosswordleCell rack[],char targetLetter)
 {	char seedLetter = targetLetter;
 	...
 	for(int lim=targetWord.length()-1; lim>=0; lim--)
 	{
 		...
 		{	boolean found = false;
 			for(CrosswordleCell rackCell : rack)
 			{
 				CrosswordleChip ch = rackCell.topChip();
 ...
 private boolean canPlaceFromRack(String targetWord,CrosswordleCell rack[],String targetString)
 {	int usedLetters = 0;
 	sweep_counter++;
 	for(int lim=targetWord.length()-1; lim>=0; lim--)
 	{
 ...
 public int checkCrossWords(DictionaryHash subDictionary,CrosswordleCell fromRack[],long letterMask,
		 		CrosswordleCell startingAt,char targetLetter,
 		...
 		{	Entry word = words.nextElement();
			 ...
			 {	 if(direction!=notInDirection)
				 ...
				 if(canPlaceFromRack(targetWord,fromRack,targetLetter))
			 	 {
				 CrosswordleCell head = canPlaceWord(targetWord,targetIndex,startingAt,direction);
				 if(head!=null)
				 {
 ...
 // both to place ordinary sprint and to place "cap words" where a letter is being added to 
 // the beginning or end of an existing word.
 public int checkExtensionWords(DictionaryHash subDictionary,CrosswordleCell fromRack[],long letterMask, Word seed)
 {	int total = 0;
 	...
 	{	Entry word = words.nextElement();
 		...
 		{	int position = targetWord.indexOf(seed.name);
 			if(position>=0 && canPlaceFromRack(targetWord,fromRack,seed.name))
 				{CrosswordleCell head = canPlaceWord(targetWord,position,seed.seed,direction);
 				if(head!=null)
 				{
 				int value = scoreWord(targetWord,head,direction,fromRack);
 ...
 private int checkCrossWords(DictionaryHash subDictionary,CellStack fromPlaces,CrosswordleCell rack[],long letterMask)
 {	
 	int total = 0;
 	for(int lim=fromPlaces.size()-1; lim>=0; lim--)
 	{
 		CrosswordleCell seed = fromPlaces.elementAt(lim);
 		CrosswordleChip top = seed.topChip();
 ...
 private int blankIndex(CrosswordleCell rack[],int from)
 {
	 for(int i=from,lim=rack.length; i<lim; i++)
	 {
		 CrosswordleCell c = rack[i];
		 if(c.topChip()==CrosswordleChip.Blank) { return(i); } 
	 }
	 return(-1);
 }
 private CrosswordleCell findChar(CrosswordleCell rack[],char ch)
 {	CrosswordleCell blank = null;
	for(int lim=rack.length-1; lim>=0; lim--)
	{	CrosswordleCell c = rack[lim];
		CrosswordleChip chip = c.topChip();
		if((chip!=null) && (chip.lcChar==ch)) { return(c); }
		if(chip==CrosswordleChip.Blank) { blank = c; } 
	}
	return(blank);
 }
 ...
 private long letterSet(CrosswordleCell rack[])
	{	long s = 0;
		for(CrosswordleCell c : rack)
		{
			CrosswordleChip ch = c.topChip();
			if(ch!=null)
 ...
 // 
 private void checkCrossWords(CellStack fromPlaces,CrosswordleCell rack[],long letterMask,double baseProgress,double progressFraction)
 ...
 private int checkCapWords(CrosswordleCell wordHead,CrosswordleCell rack[],long letterMask,boolean atStart)
 {	int total = 0;
 	for(int lim=wordHead.wordHead.size()-1; lim>=0; lim--)
	 {
		 Word w = wordHead.wordHead.elementAt(lim);
		 String name = w.name;
		 for(CrosswordleCell r : rack)
		 {
			 CrosswordleChip top = r.topChip();
			 if(top!=null)
			 {
 ...
 private int checkExtensionWords(Word w,CrosswordleCell rack[],long letterMask,boolean atStart)
 {	int total = 0;
 	String name = w.name;
 ...
 private void checkCapWords(CellStack fromPlaces,CrosswordleCell rack[],long letterMask,boolean atStart)
 {	
	 for(int lim=fromPlaces.size()-1; lim>=0; lim--)
	 {
 ...
 // look for adding multiple letters to the beginning or end of a word
 private void checkExtensionWords(WordStack words,CrosswordleCell rack[],long letterMask,boolean atStart)
 {	
 	for(int lim=words.size()-1; lim>=0; lim--)
	 {
 ...
 private void checkWords(CrosswordleCell rack[],double baseProgress,double progressScale)
 {	
	 ...
	 {	// replace the blank with each possible letter, using the
		...
		// this multiplies the search by 26x for each blank
		 double quant = progressScale/CrosswordleChip.assignedBlanks.length;
		for(CrosswordleChip letter : CrosswordleChip.assignedBlanks)
		 {
			 rack[bi].removeTop();
		 ...
		 rack[bi].removeTop();
		 rack[bi].addChip(CrosswordleChip.Blank);
	 }
	 else {
 ...
 {	candidateWords.clear();
 	for(CrosswordleCell c : rack[whoseTurn]) { c.isBlank = false; }	// just to be sure
 ...
 private double distanceFromBonuses(CrosswordleCell c)
 {	double score = 0;
 	for(CrosswordleCell b : bonusCells)
 ...
 * @return
 */
 private CrosswordleCell mostDistantFromBonuses()
 {
	 double bestv=0;
	 CrosswordleCell best = null;
 ...
 {	CommonMoveStack all = new CommonMoveStack();
    ...
    case FirstPlay:
    	...
    	CrosswordleCell best = mostDistantFromBonuses();
    	all.push(new CrosswordleMovespec(MOVE_DROPB,best.col,best.row,CrosswordleId.BoardLocation,whoseTurn));
    	}
    	break;
    case Play:
    	checkWords();
     	for(int i=0;i<candidateWords.size();i++)
     	{
     		all.push(new CrosswordleMovespec(candidateWords.elementAt(i),whoseTurn));
     	}
     	if(all.size()==0)
     	{
     		all.push(new CrosswordleMovespec(MOVE_PASS,whoseTurn));
     	}
     	break;
    }
 	recordProgress(0);
