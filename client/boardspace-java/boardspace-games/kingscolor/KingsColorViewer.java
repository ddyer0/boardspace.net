...
{	static final String KingsColor_SGF = "kingscolor"; // sgf game name
    ...
    private Color rackActiveColor = new Color(245,212,202);
  
    // private state
    private KingsColorBoard bb = null; //the board from which we are displaying
    ...
    private Rectangle captured[] = addRect("cap",2);
 ...
 */
    ...
    {	
        ...
        // for another game.
        bb = new KingsColorBoard(type,players_in_game,randomKey,getStartingColorMap(),KingsColorBoard.REVISION);
        //
    ...
    {	commonPlayer pl = getPlayerOrTemp(player);
    	...
    	Rectangle cap = captured[player];
    	G.SetRect(chip,	x,	y,	2*unitsize,	3*unitsize/2);
    	G.SetRect(cap, x,y+unitsize*3/2,unitsize,unitsize);
    ...
    private void DrawChipPool(Graphics gc, Rectangle r, Rectangle cr,commonPlayer pl, HitPoint highlight,KingsColorBoard gb)
    {	int player = pl.boardIndex;
        KingsColorChip chip = bb.getPlayerChip(player);
        prerotated = true;
        chip.drawChip(gc,this,r,null);
        int step = G.Width(cr);
        gb.captured[player].drawStack(gc,this,null,step,G.Left(cr)+step/3,G.centerY(cr)-step/4,
        		0,0.3,0,null);
        prerotated = false;
    ...
    /**
    * sprites are normally a game PieceType that is "in the air" being moved
    * around.  This is called when dragging your own pieces, and also when
    * presenting the motion of your opponent's pieces, and also during replay
    * when a PieceType is picked up and not yet placed.  While "obj" is nominally
    * a game PieceType, it is really whatever is associated with b.movingObject()
    ...
    {	KingsColorBoard gb = disB(gc);
        boolean reviewBackground = reviewMode()&&!mutable_game_record;
        if(reviewBackground)
        {	 
         KingsColorChip.backgroundReviewTile.image.tileImage(gc,brect);   
        }
	      ...
	      gb.DrawGrid(gc, brect, use_grid, rackBackGroundColor, GridColor, GridColor, GridColor);
	      ...
	      for(Enumeration<KingsColorCell> it = gb.getIterator(Itype.TBRL);
	    		  it.hasMoreElements();)
	      {
	    	  KingsColorCell c = it.nextElement();
   ...
   // }
     ...
     */
    public void drawBoardElements(Graphics gc, KingsColorBoard gb, Rectangle brect, HitPoint highlight)
    {
        //
    	...
    	boolean moving = gb.pickedObject!=null && gb.getState()!=KingsColorState.Puzzle;
    	Hashtable<KingsColorCell,KingsColormovespec> targets = gb.getTargets(who);
    	Hashtable<KingsColorCell,KingsColormovespec> secondary = null;
    	KingsColorCell dest = gb.getDest();
    	KingsColorCell src = gb.getSource();
    	if(!moving)
    	{
    	KingsColorCell close = gb.closestCell(highlight,boardRect);
    	if(close!=null && (close!=src))
    	{	KingsColorChip top = close.topChip();
    		if(top!=null)
    		{ secondary = gb.getTargets(gb.playerIndex(top.colorId),close,top);
    		}
    	}}
    	int top = G.Bottom(brect);
    	int left = G.Left(brect);
    	for(Enumeration<KingsColorCell> cells = gb.getIterator(Itype.TBLR); 
    				cells.hasMoreElements();)
    	{
    		KingsColorCell cell = cells.nextElement();
   ...
   public int getAltChipset()
    {
    	...
    	{
    	// this communicates with KingsColorChip.getChipRotation();
    	return(bb.reverseY()?1:2);
    	}
    	return(0);
    }
...
<li>stray buttons are insensitive when dragging a PieceType
...
*/
    ...
    {  
       ...
       KingsColorBoard gb = disB(gc);
       KingsColorState state = gb.getState();
       ...
       //
       // even if we can normally select things, if we have already got a PieceType
       ...
       GC.unsetRotatedContext(gc,ourTurnSelect);
       ...
       for(int player=0;player<gb.players_in_game;player++)
    	   ...
    	   DrawChipPool(gc, chipRects[player],captured[player],pl, ourTurnSelect,gb);
       ...
       GC.setFont(gc,standardBoldFont());
		...
		if (state != KingsColorState.Puzzle)
        ...
        drawPlayerStuff(gc,(state==KingsColorState.Puzzle),buttonSelect,HighlightColor,rackBackGroundColor);
        ...
        DrawReverseMarker(gc,reverseViewRect,selectPos,ColorId.Reverse);      
...
//     } 
 ...
 void playSounds(commonMove mm)
 {
	 ...
	 case MOVE_PICKB:
		 ...
		 break;
	 case MOVE_DONE:
		 if(bb.getState()==KingsColorState.Check)
	    	{
 ...
 */
    ...
    {
        return (new KingsColormovespec(st, pl));
    }
/**
 ...
 */
    ...
    {
        if (hp.hitCode instanceof ColorId)// not dragging anything yet, so maybe start
        {
        ColorId hitObject =  (ColorId)hp.hitCode;
 	    switch(hitObject)
	    {
	    default: break;
	...
	{	KingsColorState state = bb.getState();
		switch(state)
		{
 ...
 * not setting the values when the gc is null.
    ...
    public void StopDragging(HitPoint hp)
    {
        CellId id = hp.hitCode;
       	if(!(id instanceof ColorId))  {   missedOneClick = performStandardActions(hp,missedOneClick);   }
        else {
        missedOneClick = false;
        ColorId hitCode = (ColorId)id;
        
        // if direct drawing, hp.hitObject is a cell from a copy of the board
        KingsColorCell hitObject = bb.getCell(hitCell(hp));
		KingsColorState state = bb.getState();
        switch (hitCode)
        {
        default:
        	...
        	break;
        case Reverse:
        	{ boolean v = !bb.reverseY(); bb.setReverseY(v); 
          	 generalRefresh();
        	}
        	break;
    ...
    private boolean setDisplayParameters(KingsColorBoard gb,Rectangle r)
    {
 ...
 //    }
  ...
  //  }
    ...
    public String sgfGameType() { return(KingsColor_SGF); }	// this is the official SGF number assigned to the game
