...
/**
 * 
 * Squares is a shell for various games where the objective is to form a square
 * by moving pieces around on a square grid.  BGG examples include Teeko Mondrago
 * Quartetto and Oquo
 *  
*/
...
{		// move commands, actions encoded by movespecs.  Values chosen so these
    ...
    static final String Oquo_sgf = "squares"; // sgf game name

    // file names for jpeg images and masks
    static final String ImageDir = "/squares/images/";
    ...
    // private state
    private SquaresBoard bb = null; //the board from which we are displaying
 ...
 */
    ...
    {	SquaresChip.preloadImages(loader,ImageDir);	// load the images used by stones
		gameIcon = SquaresChip.Icon.image;
    }
    ...
    {	
        ...
        {	// initialize the translations when debugging, so there
        	...
        	SquaresConstants.putStrings();
        }
         
        rotationOption = myFrame.addOption("rotate board",true,deferredEvents);
        
        String type = info.getString(GameInfo.GAMETYPE, SquaresVariation.teeko.name);
        // recommended procedure is to supply players and randomkey, even for games which
        // are current strictly 2 player and no-randomization.  It will make it easier when
        // later, some variant is created, or the game code base is re purposed as the basis
        // for another game.
        bb = new SquaresBoard(type,players_in_game,randomKey,getStartingColorMap(),SquaresBoard.REVISION);
        //
 ...
 //   }
    ...
    private void DrawChipPool(Graphics gc, Rectangle r, commonPlayer pl, HitPoint highlight,SquaresBoard gb)
    {	int player = pl.boardIndex;
        ...
        { // draw a random pile of chips.  It's just for effect
            ...
            SquaresChip chip = gb.getPlayerChip(player);
            SquaresCell cell = gb.getPlayerCell(player);
            int nc = 20;	
            // draw 20 chips
    ...
    * a game piece, it is really whatever is associated with b.movingObject()
    ...
    public void drawSprite(Graphics g,int obj,int xp,int yp)
    {
    	...
    	SquaresChip.getChip(obj).drawChip(g,this,bb.cellSize(), xp, yp, null);
    }
    ...
    {	// note, drawing using disB is very important for boards which have different sizes
    	...
    	SquaresBoard gb = disB(gc);
        boolean reviewBackground = reviewMode()&&!mutable_game_record;
        if(reviewBackground)
        {	 
         SquaresChip.backgroundReviewTile.image.tileImage(gc,brect);   
        }
          ...
          SquaresChip tile = lastRotation?SquaresChip.hexTile:SquaresChip.hexTileNR;
          int left = G.Left(brect);
          int top = G.Bottom(brect);
          int xsize = gb.cellSize();//((lastRotation?0.80:0.8)*);
          for(Enumeration<SquaresCell>cells = gb.getIterator(Itype.TBRL); cells.hasMoreElements(); )
          { //where we draw the grid
        	  SquaresCell cell = cells.nextElement();
     ...
     */
    public void drawBoardElements(Graphics gc, SquaresBoard gb, Rectangle brect, HitPoint highlight)
    {
        //
        // now draw the contents of the board and highlights or ornaments.  We're also
    	// called when not actually drawing, to determine if the mouse is pointing at
    	// something which might allow an action.  
    	Hashtable<SquaresCell,SquaresMovespec> targets = gb.getTargets();
     	numberMenu.clearSequenceNumbers();

    	for(SquaresCell cell = gb.allCells; cell!=null; cell=cell.next)
          {
...
*/
    ...
    {  
       ...
       SquaresBoard gb = disB(gc);
       SquaresState state = gb.getState();
       ...
       boolean conf = (state==SquaresState.ConfirmSwap) ;
		if( conf
			|| (state==SquaresState.PlayOrSwap) 
			|| (state==SquaresState.Puzzle))
        ...
        drawPlayerStuff(gc,(state==SquaresState.Puzzle),buttonSelect,HighlightColor,rackBackGroundColor);
  
 
        // draw the avatars
        standardGameMessage(gc,messageRotation,
        					// note that gameOverMessage() is also put into the game record
            				state==SquaresState.Gameover?gameOverMessage(gb):s.get(state.description()),
            				state!=SquaresState.Puzzle,
            				gb.whoseTurn,
            				stateRect);
        gb.getPlayerChip(gb.whoseTurn).drawChip(gc,this,iconRect,null);
        goalAndProgressMessage(gc,nonDragSelect,Color.black,s.get(SimpleVictoryCondition),progressRect, goalRect);
            //      DrawRepRect(gc,pl.displayRotation,Color.black,b.Digest(),repRect);
        eyeRect.activateOnMouse = true;
        eyeRect.draw(gc,selectPos);
        DrawReverseMarker(gc,reverseRect,selectPos,SquaresId.ReverseView);
 ...
 */
    ...
    {
        return (new SquaresMovespec(st, pl));
    }
/**
 ...
 */
    ...
    {
        if (hp.hitCode instanceof SquaresId)// not dragging anything yet, so maybe start
        {
        SquaresId hitObject =  (SquaresId)hp.hitCode;
 	    switch(hitObject)
	    {
	    default: break;
 	    ...
 	    case White:
	    	...
	    	break;
	    case BoardLocation:
	        SquaresCell hitCell = hitCell(hp);
 ...
 * not setting the values when the gc is null.
    ...
    public void StopDragging(HitPoint hp)
    {
        CellId id = hp.hitCode;
       	if(!(id instanceof SquaresId))  {   missedOneClick = performStandardActions(hp,missedOneClick);   }
        else {
        missedOneClick = false;
        SquaresId hitCode = (SquaresId)id;
        
        // if direct drawing, hp.hitObject is a cell from a copy of the board
        SquaresCell hitObject = bb.getCell(hitCell(hp));
		SquaresState state = bb.getState();
        switch (hitCode)
        {
        default:
    ...
    private boolean setDisplayParameters(SquaresBoard gb,Rectangle r)
    {
 ...
 //    }
  ...
  //  }
    ...
    public String sgfGameType() { return(Oquo_sgf); }	// this is the official SGF number assigned to the game
 ...
 */
  ...
  //  }
	...
	public double imageSize(ImageStack im)
	  {
		  return(super.imageSize(im) + SquaresChip.imageSize(im));
	  }
