...
import static squares.SquaresMovespec.*;
...
/**
 * SquaresBoard knows all about the game of Squares, which is played
...
class SquaresBoard 
	extends rectBoard<SquaresCell>	// for a square grid board, this could be rectBoard or squareBoard 
	implements BoardProtocol,SquaresConstants
{	static int REVISION = 100;			// 100 represents the initial version of the game
	public int getMaxRevisionLevel() { return(REVISION); }
	static final String[] GRIDSTYLE = { "1", null, "A" }; // left and bottom numbers
	SquaresVariation variation = SquaresVariation.teeko;
	private SquaresState board_state = SquaresState.Puzzle;	
	private SquaresState unresign = null;	// remembers the orignal state when "resign" is hit
	private StateStack robotState = new StateStack();
	public SquaresState getState() { return(board_state); }
    /**
     * this is the preferred method when using the modern "enum" style of game state
     * @param st
     */
	public void setState(SquaresState st) 
	{ 	unresign = (st==SquaresState.Resign)?board_state:null;
		board_state = st;
    ...
    private SquaresId playerColor[]={SquaresId.White,SquaresId.Black};    
    private SquaresChip playerChip[]={SquaresChip.White,SquaresChip.Black};
    private SquaresCell playerCell[]=new SquaresCell[2];
    // get the chip pool and chip associated with a player.  these are not 
    // constants because of the swap rule.
	public SquaresChip getPlayerChip(int p) { return(playerChip[p]); }
	public SquaresId getPlayerColor(int p) { return(playerColor[p]); }
	public SquaresCell getPlayerCell(int p) { return(playerCell[p]); }
	public SquaresChip getCurrentPlayerChip() { return(playerChip[whoseTurn]); }
	public SquaresPlay robot = null;
	...
	public boolean p1(String msg)
		{
			...
			{	String dir = "g:/share/projects/boardspace-html/htdocs/squares/squaresgames/robot/";
...
// DrawRepRect to warn the user that repetitions have been seen.
	public void SetDrawState() { setState(SquaresState.Draw); }	CellStack animationStack = new CellStack();
    ...
    public SquaresChip pickedObject = null;
    public SquaresChip lastPicked = null;
    private SquaresCell blackChipPool = null;	// dummy source for the chip pools
    private SquaresCell whiteChipPool = null;
    ...
    private SquaresState resetState = SquaresState.Puzzle; 
    public DrawableImage<?> lastDroppedObject = null;	// for image adjustment logic

	// factory method to generate a board cell
	public SquaresCell newcell(char c,int r)
	{	return(new SquaresCell(SquaresId.BoardLocation,c,r));
	}
	
	// constructor 
    public SquaresBoard(String init,int players,long key,int map[],int rev) // default constructor
    {
	    ...
	    blackChipPool = new SquaresCell(r,SquaresId.Black);
	    blackChipPool.addChip(SquaresChip.Black);
	    whiteChipPool = new SquaresCell(r,SquaresId.White);
	    whiteChipPool.addChip(SquaresChip.White);
    ...
    {	randomKey = key;
    	...
    	win = new boolean[players];
 		setState(SquaresState.Puzzle);
		variation = SquaresVariation.findVariation(gtype);
		...
		case teeko:
			...
			reInitBoard(variation.boardSize,variation.boardSize);
	    ...
	    int map[]=getColorMap();
		playerColor[map[0]]=SquaresId.White;
		playerColor[map[1]]=SquaresId.Black;
		playerChip[map[0]]=SquaresChip.White;
		playerChip[map[1]]=SquaresChip.Black;
	    // set the initial contents of the board to all empty cells
		emptyCells.clear();
		for(SquaresCell c = allCells; c!=null; c=c.next) { c.reInit(); emptyCells.push(c); }
    ...
    public SquaresBoard cloneBoard() 
	{ SquaresBoard dup = new SquaresBoard(gametype,players_in_game,randomKey,getColorMap(),revision); 
	  dup.copyFrom(this);
	  return(dup); 
   	}
    public void copyFrom(BoardProtocol b) { copyFrom((SquaresBoard)b); }

    /* make a copy of a board.  This is used by the robot to get a copy
     * of the board for it to manipulate and analyze without affecting 
     * the board that is being displayed.
     *  */
    public void copyFrom(SquaresBoard from_b)
    {
    ...
    public void sameboard(BoardProtocol f) { sameboard((SquaresBoard)f); }
     ...
     * @param from_b
     */
    public void sameboard(SquaresBoard from_b)
    {
    ...
    public SquaresChip SetBoard(SquaresCell c,SquaresChip ch)
    {	SquaresChip old = c.topChip();
    	if(c.onBoard)
    	{
    ...
    //
    private SquaresCell unDropObject()
    {	SquaresCell rv = droppedDestStack.pop();
    ...
    {	SquaresCell rv = pickedSourceStack.pop();
    	setState(stateStack.pop());
    ...
    //
    private void dropObject(SquaresCell c)
    {
    ...
    //
    public boolean isDest(SquaresCell c)
    {	return(droppedDestStack.top()==c);
    }
    public SquaresCell getDest()
   ...
   /**
     ...
     * @return
     */
    private SquaresCell getCell(SquaresId source, char col, int row)
    {
        switch (source)
        {
        default:
     ...
     */
    public SquaresCell getCell(SquaresCell c)
    {
    ...
    private void pickObject(SquaresCell c)
    ...
    //
    public boolean isSource(SquaresCell c)
    {	return(c==pickedSourceStack.top());
    }
    public SquaresCell getSource()
    ...
    private void doDone(replayMode replay)
    {
        ...
        if (board_state==SquaresState.Resign)
        {
            win[nextPlayer[whoseTurn]] = true;
    		setState(SquaresState.Gameover);
        }
        else
...
{	SquaresId c = playerColor[0];
	SquaresChip ch = playerChip[0];
	playerColor[0]=playerColor[1];
	playerChip[0]=playerChip[1];
	playerColor[1]=c;
	playerChip[1]=ch;
	SquaresCell cc = playerCell[0];
	...
	case Play:
		...
		//$FALL-THROUGH$
	case PlayOrSwap:
		  setState(SquaresState.ConfirmSwap);
		  break;
	case ConfirmSwap:
		  setState(SquaresState.PlayOrSwap);
		  break;
	case Gameover:
	case Puzzle: break;
	}
    ...
    {	SquaresMovespec m = (SquaresMovespec)mm;
        ...
        case MOVE_DROPB:
        	{
			SquaresChip po = pickedObject;
			SquaresCell dest =  getCell(SquaresId.BoardLocation,m.to_col,m.to_row);
        ...
        case MOVE_PICK:
 		case MOVE_PICKB:
        	// come here only where there's something to pick, which must
 			{
 			SquaresCell src = getCell(m.source,m.to_col,m.to_row);
        	...
        	case Puzzle:
         		break;
        	case Confirm:
        		setState(((chips_on_board==1) && !swapped) ? SquaresState.PlayOrSwap : SquaresState.Play);
        		break;
        	default: ;
        	}}}
            break;
        ...
        case MOVE_DROP: // drop on chip pool;
        	...
        	{
            SquaresCell dest = getCell(m.source,m.to_col,m.to_row);
        ...
        case MOVE_START:
            ...
            setState(SquaresState.Puzzle);	// standardize the current state
       ...
       case MOVE_RESIGN:
    	   	setState(unresign==null?SquaresState.Resign:unresign);
            break;
       case MOVE_EDIT:
        	acceptPlacement();
            setState(SquaresState.Puzzle);
    ...
    public boolean legalToHitBoard(SquaresCell c,Hashtable<SquaresCell,SquaresMovespec> targets )
 ...
 /** assistance for the robot.  In addition to executing a move, the robot
    ...
    executing.
    */
    public void RobotExecute(SquaresMovespec m)
    {
   ...
   //
    ...
    //
    public void UnExecute(SquaresMovespec m)
    {
        //System.out.println("U "+m+" for "+whoseTurn);
    	SquaresState state = robotState.pop();
        switch (m.op)
        {
        default:
 ...
 {	CommonMoveStack all = new CommonMoveStack();
 	if(board_state==SquaresState.PlayOrSwap)
 	{
 		all.addElement(new SquaresMovespec(SWAP,whoseTurn));
 	}
 	switch(board_state)
 	{
 	case Puzzle:
 		{int op = pickedObject==null ? MOVE_DROPB : MOVE_PICKB; 	
 			for(SquaresCell c = allCells;
 			 	    c!=null;
 			 	    c = c.next)
 			 	{	if(c.topChip()==null)
 			 		{all.addElement(new SquaresMovespec(op,c.col,c.row,whoseTurn));
 			 		}
 			 	}
 		}
 		break;
 	...
 	case PlayOrSwap:
 	case Confirm:
 		all.push(new SquaresMovespec(MOVE_DONE,whoseTurn));
 		break;
 ...
 public void initRobotValues(SquaresPlay m)
 {	robot = m;
  ...
  * @return
  */
 public Hashtable<SquaresCell, SquaresMovespec> getTargets() 
 {
 	Hashtable<SquaresCell,SquaresMovespec> targets = new Hashtable<SquaresCell,SquaresMovespec>();
 	CommonMoveStack all = GetListOfMoves();
 	for(int lim=all.size()-1; lim>=0; lim--)
 	{	SquaresMovespec m = (SquaresMovespec)all.elementAt(lim);
 		switch(m.op)
 		{
 		case MOVE_PICKB:
 		case MOVE_DROPB:
 ...
 //	 return false;
	...
	something like this:
 	return (movingObjectIndex()<0)
 			&& ((board_state==SquaresState.Play) || (board_state==SquaresState.DrawPending))
