 ...
 *
 */
public class KulamiPlay extends commonRobot<KulamiBoard> implements Runnable, 
    RobotProtocol
    {
    ...
    private double TIMEPERMOVE = 4;
    private int NRANDOMMOVES = 4;
    private int MAXMOVES = 750000;
    private int boardSearchLevel = 0;				// the current search depth
    private KulamiChip movingForPlayer = null;
    /**
 ...
 */
    ...
    {	Kulamimovespec mm = (Kulamimovespec)m;
        board.UnExecute(mm);
        boardSearchLevel--;
    }
 	...
 	try {
       if (board.DoneState())
        { // avoid problems with gameover by just supplying a done
            move = new Kulamimovespec("Done", board.whoseTurn);
        }
        else 
        {
        ...
        double randomn = (RANDOMIZE && (board.moveNumber <= NRANDOMMOVES))
        ...
        monte_search_state.blitz = false;			// for Kulami, blitz is 2/3 the speed of normal unwinds
        monte_search_state.sort_moves = false;
        monte_search_state.only_child_optimization = true;
        monte_search_state.dead_child_optimization = true;
        monte_search_state.simulationsPerNode = 1;
        monte_search_state.killHopelessChildrenShare = CHILD_SHARE;
        monte_search_state.final_depth = 9999;		// note needed for Kulami which is always finite
        monte_search_state.node_expansion_rate = NODE_EXPANSION_RATE;
        monte_search_state.randomize_uct_children = true;     
        monte_search_state.maxThreads = DEPLOY_THREADS;
        monte_search_state.random_moves_per_second = WEAKBOT ? 15000 : MAXMOVES;		// 
        monte_search_state.max_random_moves_per_second = MAXMOVES;		// 
