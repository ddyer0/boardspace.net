 ...
 */
    ...
    {	Rectangle firstPlayerRect = pl0.nameRect;
    	...
    	G.SetRect(pl0.playerBox,x,y,G.Right(pic)-x,G.Bottom(pic)-y);
    }
    ...
    {	
        ...
        int ncols = tallMode ? 27 : 45; // more cells wide to allow for the aux displays
        int nrows;
        switch(nplayers)
        {
        default:
        	nrows = 18;
        	if(tallMode)  { ncols = 22; nrows = 28; }
        	break;
        case 3:
        case 4:
        	nrows = tallMode ? (noChat ? 25 : 33) : 22;
        	break;
        case 5: 
        	nrows = tallMode ? (noChat? 26 : 34) : 25;
        	if(tallMode) { ncols = playerCols*3; }
        	break;
        }
    ...
    {   
        ...
        G.SetRect(prizeRect, (G.centerX(boardRect)-5*CELLSIZE/2),
        ...
        SetupVcrRects(
        			noChat|tallMode ? width-CELLSIZE*7 : CELLSIZE,
        ...
        G.SetRect(goalRect,G.Left( boardRect)+CELLSIZE*3,		// really just a general message
        		...
        		G.Width(boardRect)-CELLSIZE*7,CELLSIZE);
        
        setProgressRect(progressRect,goalRect);

  	  int logW = CELLSIZE * 4 * nplayers;
  	  int logX = noChat ? CELLSIZE/2 : tallMode ? (nplayers==2) ? G.Left(boardRect) : CELLSIZE : width-logW-CELLSIZE/2;
      int lowest = G.Bottom(boardRect);

        {
            commonPlayer pl0 =getPlayerOrTemp(0);
            commonPlayer pl1 = getPlayerOrTemp(1);
           {
            int playerTop = G.Bottom(stateRect);
            int playerMid = playerTop + CELLSIZE*7;
            int playerLow = playerMid + CELLSIZE*7;
            ...
            if(tallMode)
            {
            	...
            	case 5:
            		createPlayerGroup(getPlayerOrTemp(4),G.Left(boardRect)+playerW,playerBot,playerRects[4],true,false);
					//$FALL-THROUGH$
				case 4:
            		commonPlayer pl3 =getPlayerOrTemp(3); 
            		createPlayerGroup(pl3,G.Left(boardRect),playerBot,playerRects[3],true,false);
            		lowest = Math.max(lowest, G.Bottom(pl3.playerBox));
					//$FALL-THROUGH$
				case 3:
            		commonPlayer pl2 = getPlayerOrTemp(2);
               		createPlayerGroup(pl2,playerLeft,playerLow,playerRects[2],true,false);
               		createPlayerGroup(pl1,playerLeft,playerMid,playerRects[1],true,false);
                    createPlayerGroup(pl0,playerLeft,playerTop,playerRects[0],true,false);
                    lowest = Math.max(lowest,G.Bottom(pl2.playerBox));
                    break;
            	case 2:
            		createPlayerGroup(pl1,playerLeft+playerW+CELLSIZE,playerBot,playerRects[1],true,false);
                    createPlayerGroup(pl0,playerLeft,playerBot,playerRects[0],true,false);
                    lowest = Math.max(lowest,G.Bottom(pl1.playerBox));
            ...
            if(nplayers>2)
            {	commonPlayer pl2 = getPlayerOrTemp(2);
            	createPlayerGroup(pl2,playerRight,playerMid,playerRects[2],false,false);
            	lowest = Math.max(lowest,G.Bottom(pl1.playerBox));
              	if(nplayers>3)
            	{	commonPlayer pl3 = getPlayerOrTemp(3);
            			{ if(nplayers>4)
            				{
            				createPlayerGroup(pl3,G.Left(boardRect),playerBot,playerRects[3],true,true);
            				}
            			else
            				{
            				createPlayerGroup(pl3,playerLeft,playerMid,playerRects[3],true,false);  
             				}
            		lowest = Math.max(lowest,G.Bottom(pl3.playerBox));
          		}
            		if(nplayers>4)
            		{	commonPlayer pl4 = getPlayerOrTemp(4);
            		 	createPlayerGroup(pl4,playerLeft,playerMid,playerRects[4],true,false);  
               			lowest = Math.max(lowest,G.Bottom(pl4.playerBox));
            		}
            	}
            }}
            ...
            int chatY = y;
            G.SetRect(chatRect, 
            		x+CELLSIZE/2,		// the chat area
            		chatY,
            		tallMode ? width-CELLSIZE : logX-CELLSIZE,
            		wideMode?Math.min(height-chatY-CELLSIZE/2,chatHeight):chatHeight);
	  ...
	  int logY = noChat||tallMode ? lowest+CELLSIZE/2 : y;
	  int logHeight = noChat|tallMode ? height-logY-CELLSIZE : chatHeight;
