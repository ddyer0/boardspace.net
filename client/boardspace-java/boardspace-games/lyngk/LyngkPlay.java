 ...
 *
 */
public class LyngkPlay extends commonRobot<LyngkBoard> implements Runnable,
    RobotProtocol
    {
    ...
    private boolean FASTRANDOM = true;
    ...
    {	RobotProtocol c = super.copyPlayer(from);
    	LyngkPlay cc = (LyngkPlay)c;
    	cc.Strategy = Strategy;
    	cc.useUCTPriors = useUCTPriors;
    	cc.FASTRANDOM = FASTRANDOM;
    	return(c);
    }
 	...
 	try {
       if (board.DoneState())
        { // avoid problems with gameover by just supplying a done
            move = new LyngkMovespec(MOVE_DONE, board.whoseTurn);
        }
        else 
        {
        ...
        monte_search_state.final_depth = 9999;		// not needed for lyngk which is always finite
        monte_search_state.node_expansion_rate = NODE_EXPANSION_RATE;
        monte_search_state.randomize_uct_children = true;     
        monte_search_state.maxThreads = DEPLOY_THREADS;
        monte_search_state.random_moves_per_second = WEAKBOT ? 1000 : 380000;		// 
