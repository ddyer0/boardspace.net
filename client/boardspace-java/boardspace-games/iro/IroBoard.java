...
class IroBoard 
	extends hexBoard<IroCell>	// for a square grid board, this could be rectBoard or squareBoard 
...
{	static int REVISION = 100;			// 100 represents the initial version of the game
    ...
    private IroChip playerChip[]={IroChip.White,IroChip.Black};
...
// DrawRepRect to warn the user that repetitions have been seen.
	...
	{	return(new IroCell(PrototypeId.BoardLocation,c,r));
	}
    ...
    public IroBoard(String init,int players,long key,int map[],int rev) // default constructor
    {
	    ...
	    blackChipPool = new IroCell(r,PrototypeId.Black);
	    blackChipPool.addChip(IroChip.Black);
	    whiteChipPool = new IroCell(r,PrototypeId.White);
	    whiteChipPool.addChip(IroChip.White);
    ...
    {	randomKey = key;
		...
		playerChip[map[0]]=IroChip.White;
		playerChip[map[1]]=IroChip.Black;
	    // set the initial contents of the board to all empty cells
		emptyCells.clear();
		for(IroCell c = allCells; c!=null; c=c.next) { c.reInit(); emptyCells.push(c); }
    ...
    //
    private void dropObject(IroCell c)
    {
   ...
   /**
     ...
     * @return
     */
    private IroCell getCell(PrototypeId source, char col, int row)
    {
        switch (source)
        {
        default:
    ...
    private void pickObject(IroCell c)
...
{	PrototypeId c = playerColor[0];
    ...
    {	Iromovespec m = (Iromovespec)mm;
        ...
        case MOVE_DROPB:
			...
			IroCell dest =  getCell(PrototypeId.BoardLocation,m.to_col,m.to_row);
 ...
 {	CommonMoveStack all = new CommonMoveStack();
 	...
 	{
 		all.addElement(new Iromovespec(SWAP,whoseTurn));
 	}
 	switch(board_state)
 	{
 	case Puzzle:
 		{int op = pickedObject==null ? MOVE_DROPB : MOVE_PICKB; 	
 			for(IroCell c = allCells;
 			 	    c!=null;
 			 	    c = c.next)
 			 	{	if(c.topChip()==null)
 			 		{all.addElement(new Iromovespec(op,c.col,c.row,whoseTurn));
 			 		}
 			 	}
 		}
 		break;
 	...
 	case PlayOrSwap:
 	case Confirm:
 		all.push(new Iromovespec(MOVE_DONE,whoseTurn));
 		break;
