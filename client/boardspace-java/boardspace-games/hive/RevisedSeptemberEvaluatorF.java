 ...
 * 
 * revision second D increases the bonus for pillbug adjacend to Q
 ...
 */
class RevisedSeptemberEvaluatorF extends DefaultEvaluator implements Evaluator
{	
	...
	{	
		...
		if(pillbugAdjacent) {	credits -= friendlyPillbugAtQueenCount; }
		...
		for(int i=0,lim=board.occupiedCells.size(); i<lim; i++)
			{	HiveCell loc = board.occupiedCells.elementAt(i);
				...
				{	
					...
					else if((crude_queen_distance<=1) && bug.color==targetColor)
					{
