 ...
 */
class RevisedSeptemberEvaluatorH extends DefaultEvaluator implements Evaluator
{	
	...
	private double DropWeight = 5;
	private double dropValue[] = {
		// index by distance to Q
		2,2,1.5,1,
	};
	...
	double pinPenalty[] = 
		...
		0.15,			// ladybug
		0.1,			// original pillbug
		0.0,			// pillbug, combined with queen affinity gets it DOWN
		0.0};			// blank
	...
	private double friendlyMobileAtQueen = 0.25;	// partial credit for friendlies that can be moved
	// TODO negate the values and the uses, avoid future negative number screwups
	double queen_safety[] = { 0.0, -5, -10.0, -15.0, -40.0, -80.0, -120,-130,-140,-150};
	...
	// to avoid draw loops, these must be >= pinPenalty
	double burial_penalty[] =
		{Math.max(pinPenalty[0]*PinPenaltyWeight,2.0),			// queen
		 Math.max(pinPenalty[1]*PinPenaltyWeight,0.5),			// ant	
		 Math.max(pinPenalty[2]*PinPenaltyWeight,3.0),			// grasshopper
		 Math.max(pinPenalty[3]*PinPenaltyWeight,5.0),			// beetle
		 Math.max(pinPenalty[4]*PinPenaltyWeight,0.13),			// spider
		 Math.max(pinPenalty[5]*PinPenaltyWeight,5.0),			// mosquito
		 Math.max(pinPenalty[6]*PinPenaltyWeight,3.0),			// ladybug
		 Math.max(pinPenalty[7]*PinPenaltyWeight,1.0),			// original pillbug
		 Math.max(pinPenalty[8]*PinPenaltyWeight,15.0),			// pillbug
		 Math.max(pinPenalty[9]*PinPenaltyWeight,0.0)};			// blank
    ...
    {	
    	...
    	case 1:	// second move
					...
					{ HiveCell ca = c.exitTo(dir);
					  if(ca.sweep_counter!=sweep) 
					  	{ if(ca.height()==0  && board.legalToDropAdjacent(ca,targetColor))
					  		{int dis = board.hex_cell_distance(ca,q);
					  		 val = Math.max(val,DropWeight*dropValue[Math.min(dis,dropValue.length-1)]);
					  		}
     	...
     	if(val==0) { val -= nodropPenalty; }
     	return val;
    }
  
	// return the number of adjacent cells owned by a player
	public double nOccupiedOrOwnedAdjacent(HiveCell center,HiveId targetColor)
	{	double n=0;
		HivePiece bug = center.topChip();
		if(bug.color!=targetColor) { n += 0.75; }	// someone else on top

		for(int lim=center.geometry.n-1;lim>=0;lim--)
		{ HiveCell c = center.exitTo(lim);
		  int h = c.height();
		  if(c!=null && h>0) 
		  	{ n++; 
		  	  if(h>1)
		  	  {
		  		  ...
		  		  if(top.color!=targetColor) 
		  			  ...
		  			  HiveCell l1 = center.exitTo(lim-1);
		  			  HiveCell l2 = center.exitTo(lim+1);
    ...
    public boolean sibMobileAny(HiveGameBoard b,HiveCell loc)
    {	if(loc.height()==1)
    	{
    	for(int dir = b.geometry.n-1; dir>=0; dir--)
    	{
    		HiveCell sib = loc.exitTo(dir) ;
    		if(sib.height()==1)
    		{	if(b.validHiveMovegen(sib) && !b.validHive2(loc,sib)) { return(true); }
    		}
    		}
    	}
    	return(false);
	...
	{ 	HiveGameBoard board = (HiveGameBoard)boardp;
		...
		if(some)
			{ // a bonus or deficit if there's no place to put a bug
			  ...
			  if(print) { msg = "res "+msg+" = "+val+"  drop="+nv;}
			  val += nv ;
			}
			else
			...
			{	HiveCell loc = board.occupiedCells.elementAt(i);
				...
				{	int pieceordinal = bugtype.ordinal();
					...
					if(top)
					{
						...
						if(!some && (bugtype==PieceType.BEETLE 
						...
						boolean sibmo = sibMobileAny(board,loc);
						if(sibmo)
							{mobilemul=mobilemul/2;			// penalizes pairing movable pieces
					...
					{	if(loc.isAdjacentToDanger(bug.color))
						...
						case 2:
							...
							val += pillbug_queen_afinity_weight/2;
						}
					}
