...
{	
	...
	private double totalChoicesWeight = 0.5;			// benefit of any choice at all.
	...
	double queen_safety[] = { 0.0, -5, -10.0, -15.0, -40.0, -65.0, -120,-120,-120,-120};
	...
	{ 	HiveGameBoard board = (HiveGameBoard)boardp;
		...
		for(int i=0,lim=board.occupiedCells.size(); i<lim; i++)
			{	HiveCell loc = board.occupiedCells.elementAt(i);
				...
				if(top && loc==myQueenLoc && bug.color!=targetColor) { beetleOnTop = 1; }	// someone else on top
		...
		{	// evaluate queen safety, considering friendlies that can move away
			int occ = myQueenLoc.nOccupiedOrOwnedAdjacent(targetColor);
			int na = occ+beetleOnTop;
			if(na>0)
			{
			...
			{ double dif = (queen_safety[na-1]-saf)*(pillbugAdjacent ? 1.0 : friendlyMobileAtQueen);
			  saf += dif;	// partial credit
			  if(print) { msg += " mob cred "+dif; }
			}
			if(print) { msg += " qsaf "+na+"="+(saf+topval); }
			val += saf+topval;
			}
		}
