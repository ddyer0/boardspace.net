 ...
 */
class RevisedSeptemberEvaluatorW extends DefaultEvaluator implements Evaluator
{	
	...
	private double trustYourGutThreshold = 25;			// full evaluation changes less than this 
	private double trustYourGutTrigger = -50;			// absolute score less than this, so the game looks like a draw
	...
	private double QueenDropCount = 0.25;	// count for drop space adjacent to enemy Q. 
											...
											// refer to the standard development of start-07-s
	private double enemyBeetleAtQueenCount = 0.75;
	private double friendlyPillbugAtQueenCount = 0.5;	// partial credit for a friendly pillbug
	private double friendlyMobileAtQueenCount = 0.5;	// partial credit for friendlies that can be moved
	...
	{	double n=0;
		...
		if(enemyBeetle) { n += enemyBeetleAtQueenCount; }	// someone else on top
		HiveCell prev = null;
		...
		{ HiveCell c = queen.exitTo(lim);
		  	...
		  	{ n++; 
		  	  ...
		  	  if(top.color==targetColor) 
		  		  ...
		  		  if( ( (top.type==PieceType.PILLBUG)
		  				  ...
		  				  && (c.nOccupiedAdjacent()<6))
		  			  { pillbugAdjacent =true;			  
		  			  }
		  	  	}
		  	  else if(h>1)
		  ...
		  else if(enemyBeetle)
		  {
			...
			{
				n+=QueenDropCount;	// drop destination directly next to Q
			}
		  }
		  prev = c;
		}
		if(myQueenAdj || pillbugAdjacent)
		{ double dif = (pillbugAdjacent ? friendlyPillbugAtQueenCount : friendlyMobileAtQueenCount);
		  n -= dif;
		}	
		return(n);
	}	
...
//		if(pl==0) { return -100; }
			...
			{	HiveCell loc = board.occupiedCells.elementAt(i);
				...
				else // opponent color
				{
					...
					{	// buried bugs get the same penalty as pinned bugs. If more then the beetles won't leave!
						...
						if(print) { msg += " "+bug.exactBugName()+"[ under="+ pen+"] "; }
						plusPoints += pen;
	...
	{ 
      commonMove move = m;  
  	  if(Math.abs(eval)<trustYourGutTrigger)
  	  {
