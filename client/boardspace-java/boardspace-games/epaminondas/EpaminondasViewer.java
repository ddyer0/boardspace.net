 ...
 *  <p>
 *  The primary purpose of the EpaminondasViewer class is to do the actual
 ...
 *  
*/
public class EpaminondasViewer extends CCanvas<EpaminondasCell,EpaminondasBoard> implements EpaminondasConstants, PlacementProvider
{		// move commands, actions encoded by movespecs.  Values chosen so these
    // integers won't look quite like all the other integers
 	
    static final String Epaminondas_SGF = "epaminondas"; // sgf game name

    // file names for jpeg images and masks
    static final String ImageDir = "/epaminondas/images/";
    ...
    // private state
    private EpaminondasBoard bb = null; //the board from which we are displaying
    ...
    private Toggle eyeRect = new Toggle(this,"eye",
 			StockArt.NoEye,EpaminondasId.ToggleEye,NoeyeExplanation,
 			StockArt.Eye,EpaminondasId.ToggleEye,EyeExplanation
 			);
 ...
 */
    ...
    {	EpaminondasChip.preloadImages(loader,ImageDir);	// load the images used by stones
    ...
    {	
        ...
        String type = info.getString(GameInfo.GAMETYPE, EpaminondasVariation.epaminondas.name);
        // recommended procedure is to supply players and randomkey, even for games which
        // are current strictly 2 player and no-randomization.  It will make it easier when
        // later, some variant is created, or the game code base is re purposed as the basis
        // for another game.
        bb = new EpaminondasBoard(type,players_in_game,randomKey,getStartingColorMap(),EpaminondasBoard.REVISION);
        //
 ...
 //   }
    ...
    public void setLocalBounds(int x, int y, int width, int height)
    {
        ...
        int stateH = fh*5/2;    	
        int nrows = bb.variation.nrows;
        int ncols = bb.variation.ncols;
    ...
    {	commonPlayer pl = getPlayerOrTemp(player);
    	...
    	G.SetRect(chip,	x,	y,	unitsize,	unitsize);
    	Rectangle box =  pl.createRectangularPictureGroup(x+unitsize,y,2*unitsize/3);
    ...
    private void DrawChipPool(Graphics gc, EpaminondasBoard gb, Rectangle r,
    		commonPlayer pl, HitPoint highlight,   	Hashtable<EpaminondasCell,EpaminondasMovespec> targets)
    {	int player = pl.boardIndex;
    	EpaminondasCell c = gb.getPlayerCell(player);
        EpaminondasMovespec move = targets.get(c);
        int count = gb.getChipCount(player);
        if(c.drawStack(gc,this,move==null ? null : highlight,G.Width(r),G.centerX(r),G.centerY(r),0,0,""+count))
        {
        	highlight.hitData = move;
        }
    }
    /**
    ...
    * a game piece, it is really whatever is associated with b.movingObject()
    ...
    {	// note, drawing using disB is very important for boards which have different sizes
    	...
    	EpaminondasBoard gb = disB(gc);
        boolean reviewBackground = reviewMode()&&!mutable_game_record;
        if(reviewBackground)
        {	 
         EpaminondasChip.backgroundReviewTile.image.tileImage(gc,brect);   
        }
          ...
          int xsize = (int)(gb.cellSize()*1.1);
          int ysize = (int)(gb.cellSize()*1.11);
          for(Enumeration<EpaminondasCell>cells = gb.getIterator(Itype.TBRL); cells.hasMoreElements(); )
          { //where we draw the grid
        	  EpaminondasCell cell = cells.nextElement();
        	  int ypos = top - gb.cellToY(cell);
        	  int xpos = left + gb.cellToX(cell);
        	  int thiscol = cell.col;
        	  int thisrow = cell.row;
        	  GC.fillRect(gc,xpos-ysize/2,ypos-ysize/2,ysize,ysize);
        	  EpaminondasChip tile = EpaminondasChip.getTile((thiscol^thisrow)&1);
     ...
     */
    public void drawBoardElements(Graphics gc, EpaminondasBoard gb, Rectangle brect, HitPoint highlight,Hashtable<EpaminondasCell,EpaminondasMovespec> targets)
    {
        //
    	...
    	// something which might allow an action.  
     	...
     	boolean show = (eyeRect.isOnNow() && gb.pickedObject!=null);
    	for(EpaminondasCell cell = gb.allCells; cell!=null; cell=cell.next)
          {
         	int ypos = G.Bottom(brect) - gb.cellToY(cell);
            int xpos = G.Left(brect) + gb.cellToX(cell);
            numberMenu.saveSequenceNumber(cell,xpos,ypos);
            EpaminondasMovespec move = targets.get(cell);
            boolean canHit = move!=null;
...
*/
    ...
    {  
       ...
       EpaminondasBoard gb = disB(gc);
       EpaminondasState state = gb.getState();
       ...
       Hashtable<EpaminondasCell,EpaminondasMovespec> targets = gb.getTargets();
       ...
       for(int player=0;player<gb.players_in_game;player++)
    	   ...
    	   DrawChipPool(gc,gb,chipRects[player],pl, ourTurnSelect,targets);
       ...
       if (state != EpaminondasState.Puzzle)
        ...
        drawPlayerStuff(gc,(state==EpaminondasState.Puzzle),buttonSelect,HighlightColor,rackBackGroundColor);
  
 
        // draw the avatars
        standardGameMessage(gc,messageRotation,
        					// note that gameOverMessage() is also put into the game record
            				state==EpaminondasState.Gameover?gameOverMessage(gb):s.get(state.description()),
            				state!=EpaminondasState.Puzzle,
            				gb.whoseTurn,
            				stateRect);
        ...
        DrawReverseMarker(gc,reverseRect,selectPos,EpaminondasId.ReverseView);
 ...
 */
    ...
    {
        return (new EpaminondasMovespec(st, pl));
    }
/**
 ...
 */
    ...
    {
        if (hp.hitCode instanceof EpaminondasId)// not dragging anything yet, so maybe start
        {
        EpaminondasId hitObject =  (EpaminondasId)hp.hitCode;
 	    switch(hitObject)
	    {
	    default: break;
 	    ...
 	    case White:
	    	...
	    	break;
	    case BoardLocation:
        	EpaminondasMovespec move = (EpaminondasMovespec)hp.hitData;
 ...
 * not setting the values when the gc is null.
    ...
    public void StopDragging(HitPoint hp)
    {
        CellId id = hp.hitCode;
       	if(!(id instanceof EpaminondasId))  {   missedOneClick = performStandardActions(hp,missedOneClick);   }
        else {
        missedOneClick = false;
        EpaminondasId hitCode = (EpaminondasId)id;
        
        // if direct drawing, hp.hitObject is a cell from a copy of the board
        EpaminondasCell hitObject = bb.getCell(hitCell(hp));
        switch (hitCode)
        {
        default:
    ...
    private boolean setDisplayParameters(EpaminondasBoard gb,Rectangle r)
    {
 ...
 //    }
  ...
  //  }
    ...
    public String sgfGameType() { return(Epaminondas_SGF); }	// this is the official SGF number assigned to the game
 ...
 */
   ...
   // {	// this is what the standard method does:
	...
	public double imageSize(ImageStack im)
	  {
		  return(super.imageSize(im) + EpaminondasChip.imageSize(im));
	  }
