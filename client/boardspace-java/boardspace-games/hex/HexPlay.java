...
/** 
 * Hex uses both alpha-beta and mcts
 * 
 ...
 *
 */
public class HexPlay extends commonRobot<HexGameBoard> implements Runnable,
    RobotProtocol
    {
    ...
    private hexChip movingForPlayer = null;
 ...
 */
    ...
    * @param evboard
    ...
    {
            HexGameBoard evboard = GameBoard.cloneBoard();
 	...
 	try {
       if (board.DoneState())
        ...
        if(move!=null && board.moveNumber>board.ncols)
        {
        	...
        	if(winrate<-0.25)
        	{	move = new Hexmovespec(RESIGN,board.whoseTurn);  
        	}
        	}
