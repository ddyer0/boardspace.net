...
class BlobStack extends OStack<XehBlob>
{
	public XehBlob[] newComponentArray(int n) { return(new XehBlob[n]); }
}
 ...
 *
 */
public class XehBlob implements XehConstants
{ XehChip color;	// W or B chip
  XehCell cells;	// linked list of cells in this blob
  char leftcol;		// bounding box in board coordinates
  int toprow;
  char rightcol;
  int bottomrow;
  XehConnection connections;	// connections to other blobs (ie shared liberties)
  XehBlob mergedWith=null;		// linked list of other blobs we're merged with
  
  // constructor
  public XehBlob(XehChip col) { color = col; }
 ...
 // care when the number of connections is two or more.
  public void addConnection(XehBlob to,XehCell cell)
  { 
	for(XehConnection conn=connections;  conn!=null;  conn=conn.next)
	{	if((conn.to==to) && (conn.through==cell)) return;
	}
	connections = new XehConnection(to,cell,connections);
  }
    ...
    {  // playerChar records what color the player is playing
    	if(color==XehChip.White) { return(rightcol-leftcol+1); }
    	if(color==XehChip.Black) { return(bottomrow-toprow+1); }
    	throw G.Error("Not expecting this");
    }

    // count the number of cells that connect two blobs
    int numConnTo(XehBlob to)
    {
      XehConnection conn = connections;
      int val = 0;
      while(conn!=null)
      { if(to==conn.to) { val++; }
        conn = conn.next;
    ...
    void mergeBlob(XehBlob mergeto)
    { 
      ...
      connections = new XehConnection(mergeto,null,connections);    
      // recursively merge other blobs
      for(XehConnection conn=mergeto.connections; conn!=null; conn=conn.next)
    ...
    static BlobStack mergeBlobs(OStack<XehBlob> blobs)
    {	BlobStack newblobs = new BlobStack();
    	for(int i=0; i<blobs.size(); i++)
    	{	XehBlob cb = blobs.elementAt(i);
    		if(cb.mergedWith==null)
    		{
    		XehBlob newb = new XehBlob(cb.color);
    		newblobs.push(newb);
    		newb.mergeBlob(cb);
    		}
     	}
