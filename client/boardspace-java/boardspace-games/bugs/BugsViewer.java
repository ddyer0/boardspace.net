 ...
 */
    ...
    {	
        ...
        String type = info.getString(GameInfo.GAMETYPE, BugsVariation.bugspiel.name);
 ...
 //   }
    ...
    {	int player = pl.boardIndex;
        ...
        { // draw a random pile of chips.  It's just for effect
            ...
            BugsChip chip = gb.getPlayerChip(player);
            BugsCell cell = gb.getPlayerCell(player);
            int nc = 20;	
            // draw 20 chips
...
*/
    ...
    {  
       ...
       boolean conf = (state==BugsState.ConfirmSwap) ;
        ...
        drawPlayerStuff(gc,(state==BugsState.Puzzle),buttonSelect,HighlightColor,rackBackGroundColor);
  
 
        // draw the avatars
        standardGameMessage(gc,messageRotation,
        					// note that gameOverMessage() is also put into the game record
            				state==BugsState.Gameover?gameOverMessage(gb):s.get(state.description()),
            				state!=BugsState.Puzzle,
            				gb.whoseTurn,
            				stateRect);
